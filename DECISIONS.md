...
**Context:** Peter (18 Sep 2026) wants ezCater items matched BEFORE any order, not pasted by hand and not left to ezCater's menu team. The connected token can read menus (proven live, read only). The full sync on `feat/ezcater-menu-sync` routed wrong sizes and wrong items in review. Peter chose the simpler version. Three review rounds of this version kept finding loose name matches at order time, and round 3 found that rows keyed by a folded name let two ezCater products share one row, its ids and its decision. Review round 4 (18 Sep 2026) made matching exact by construction. Rounds 4 and 5 then each found a new way for two "exact" names to fold together, so review round 6 (19 Sep 2026) narrowed what the sync does on its own instead of perfecting the folding. The menu sync ships as its own release, after the ezCater orders release (v5.9.9).

**The rule, in plain words:** the sync links plain item names that match exactly; options, and names with symbols or emoji, are for staff to match. After the sync is set up, orders only use matches made before the order: plain item names the sync linked, or matches staff saved, and only for the exact name and size they were made for. There is no name guessing at order time. Anything else prints by name.

**Decision:**
- **Exact by construction.** Every row a sync writes is keyed by the exact full name of ONE ezCater product: `exact:` plus `exactName(item name + size)` (a multi size item's size, or a single size item's only size), or for an option `exact:` plus the exact names of the item it customizes, the group and the value as a JSON array (item scoped since review round 5, structured since round 6). `exactName` (round 6) folds a PLAIN name by case, accents on Latin letters and whitespace only, and keeps any other name exactly as written; no tray, pan, box, serves, bracketed part, punctuation or size word is dropped. So two ezCater products whose names differ by any word never share a row, its published ids (`ez_ids`) or a decision. Under the old folded keys "Caesar Salad" and "Caesar Salad (Serves 20)", "Sandwich Platter" and "Sandwich Platter Tray", "Sandwich Platter (Large)" and "Sandwich Platter (Small)", two sizes with no name, and "Bread: White" and "Bread Size: White" each shared one row. A row's exact full name never changes, so its ids are only ever added (new first, 200 kept); a rename on ezCater is a new row. The same exact name on two items or two menus is one product by name: one row, both ids (Potbelly's live menu lists "Bottled Water" under two categories). Two sizes of ONE item with the same exact name get no row at all, because no name can say which one an order means; the sync message names them.
- **Item scoped options (review round 5, 19 Sep 2026).** Round 5 also made `exactName` Unicode aware (NFKD, fractions read as digits, symbols as words); round 6 replaced that rule (below) because it still folded flag emoji, keycaps, superscripts, '%' and the Cyrillic breve together. An option row is keyed by its item, group and value, and carries the item's name (`ez_item_name`, new in 20260919m), so one staff match on "Size: Large" routes that customization only on the item it was made for; at order time the key is built from the name of the line the customization is on. A staff option decision from before (a row saved before the sync, or an option row a sync keyed without its item) is kept as it was only when its group and value are exactly the row's AND a whole read of ezCater's current menus shows exactly one item offering them; otherwise it is carried for staff to look at again. An item row a sync keyed under the ASCII rule passes its decision on through `decided_as`, so it is kept only for the name it was made for. Earlier option rows are never written, never route, and the card lists them with the rows from before the sync.
- **Only plain names link on their own (review round 6, 19 Sep 2026).** The sync links plain item names that match exactly; options, and names with symbols or emoji, are for staff to match. A name is PLAIN when it holds nothing but letters and digits from any script, spaces, the ordinary punctuation `. , ' & ( ) - /` and accents: no emoji, symbol, '%', superscript, fraction character, format, private use, tag, enclosing or variation character. An item auto links only when BOTH full names (theirs with its size or only size, and ours) are plain and are the same after folding case, accents on Latin letters and whitespace, nothing else ("Mac & Cheese" is not "Mac and Cheese", "Чай" is not "Чаи", full width letters are not ASCII). A name that is not plain is keyed by its text as ezCater wrote it (composed form, whitespace evened out), so the Welsh and the Scottish flag, two keycaps, "Pizza 10²" and "Pizza 102", "Milk 2%" and "Milk 2", "Ziti ½ Pan" and "Ziti ¼ Pan", "Pho 大" and "Pho 小" are separate rows that staff match one by one; a staff match on one routes that exact text only. OPTIONS ARE NEVER AUTO LINKED: the sync lists every option row (its item, group and value) for staff, writes no automatic option decision, clears one if it finds it, and an automatic option decision never routes (not even one a round 5 sync wrote, from the moment this code runs). An option's key and its `decided_as` hold the item, group and value apart (JSON), never parsed back out of a display string, so a group like "Step 1: Choose Bread" can be matched, confirmed with "Still right" and routed. A staff option decision from before keeps the round 5 carry rules, compared as text with what the person saw. A decision from an earlier sync is carried on EVERY later sync, not only the first insert: from a row that now belongs to another name, only when its `decided_as` is exactly the new row's name.
...
- **Auto links (`matched_by 'exact'`, items only).** An item auto links only when its full name is plain and equals exactly one of our plain item names (case, Latin accents and whitespace aside): the item plus its size, where a single size item's only size counts unless every word of it is already in the item name ("Italian Boxed Lunch" sold as "Box"; that allowance is in the auto link comparison only, never in a key). The size with no name of an item with several sizes never auto links. An option never auto links (round 6). Every sync decides every automatic row again with our whole menu: the one exact match (kept, moved, or set) or cleared, guarded on the values read, so a person who saved first wins. An automatic row off the current menu keeps its link only while its stored name is still exactly the one item of ours it points at. With our menu read only in part nothing is linked, moved or cleared.
...
- **Order time, after 20260919m (`planSyncedLineMatches`):** a line matches ONLY when `exactName(its name + its size name)` IS a synced row's exact full name, AND its published size id is on that row, AND that row holds a trusted decision: a staff match made for that name, or an exact auto link from a sync on a plain item name keyed by today's rule. The order's `menuItemSizeId` IS the menu's `sizes.id` (proven on HKX77V, 18 Sep 2026); the order's item id is never the menu's item id and is never matched. A customization matches only by a staff match, on the row of the exact name of its line (the item), its exact group and value, AND its published id (`customizationId`, not yet seen on a live order: if it is not the menu's value id, no customization ever matches and each prints by name). The exact name check can only ever match fewer lines than the id alone, and it closes the window after ezCater renames a size in place (same id, new name) before the next sync. No name guessing at order time: not `autoLinkDecision`, not the saved name links, not the legacy key fallback, not a folded name, not a posItemId, not an item code. Every other line prints by name, and an order writes nothing but the seen counters of the synced rows it landed on. The card stops offering item codes for ezCater.
...
- **Saves.** After 20260919m the card saves a synced row by its key and the edge function only UPDATES it (never creates one), recording what the screen showed as `decided_as` (an option's item, group and value apart). A save of any other row after the migration, including an option row keyed by an earlier round, comes from a Back Office tab loaded before it and is refused: "This page is out of date. Reload it, then match again." Before the migration a save is exactly main's.
...
Migration: `20260919m_OPS_ezcater_menu_sync_v1.sql` (renamed: its first name clashed with a parked branch; `decided_as` added in review round 3; no schema change in round 4, keys are text; `ez_item_name` added in round 5, so a copy run before round 5 must be run again; no schema change in round 6, only the text of new keys), run AFTER ezcater-connect and ezcater-webhook are deployed, outside service, then press Sync.

---

## ADR-025: One neutral accounting day, shared by Xero and QuickBooks (v5.9.11)

**Context:** QuickBooks planning (19 Sep 2026) found xero-sales booked a UTC day (UK after-midnight trade and BST's first hour on the wrong day), dropped split tenders (`closed_checks` said only 'split'), dated refunds by the check's close, sent unmapped tips and service to revenue, deleted its sync log on every attempt, could re-post a half posted day, and had no token refresh lock.

**Decision:**
- **The day is the venue business day:** platform `locations.timezone` plus `business_day_start` (default 06:00, the setting the Back Office sales reports use), DST safe (`_shared/businessDay.js`). A day that has not ended is never posted. The nightly job runs hourly (migration 20260919n) and books each venue's last day that ended at least 4 hours ago, so offline tills catch up first; a person asking again for a posted day is told if it has changed since.
- **The change over from UTC days:** a day whose previous day was posted by the old code (an 'ok' row with no postings and no venue clock) starts at 00:00Z, where that UTC day ended, so no trade is missed or posted twice.
- **Money is summed per tender** by a neutral aggregator (`_shared/accountingDay.js`, pure JS, loaded by `npm test` as shipped). It knows nothing about Xero: `_shared/xeroPostingPlan.js` turns the day into Xero transactions, and QuickBooks will write its own planner over the same summary.
- **`closed_checks.tenders`** (`[{method, amount, tip, gift_card_id?, psp_ref?, processor?}]`, major units) lists EVERYTHING that settled the bill, money or not, so it does not depend on whether a surface's `total` is gross (till) or net of credits (kiosk, online, terminal jobs). Built at the moment of payment by `src/lib/accounting/tenders.js`. Loyalty and promo credit are tenders of kind `discount`: never posted as takings.
- **Older rows** fall back to what the row itself proves: the method column; the online card path's `payment_method` list; the till's `gift_card+card` composite (gift card from `gift_card.applied`, booking credit from `payment_intents`); kiosk rows from their own gift card, loyalty and promo fields. An older split, or a composite with a loyalty or promo credit the row never stored, is ONE `unallocated` tender, posted to ServOS Unallocated Clearing and flagged.
- **The kiosk writes no tenders** (its card path is fingerprint guarded, `kioskCardPathGuard.test.js`); its rows are read exactly from their own fields. Adding tenders to the kiosk write needs Peter's sign off and a hardware test.
- **Refunds belong to the day of the refund** (`refunds[].timestamp`), with their own tip, service and tax, placed on the tender the money went back to (card legs, a full refund's gift card and credit, cash when staff chose cash, else the check's tenders in proportion). Failed refunds are not money.
- **Xero:** per clearing account one Receive Money (takings) and one Spend Money (refunds). Default clearing account per tender kind (card, cash, gift card, deposits, unallocated). Tips default to ServOS Tips Payable and service to ServOS Service Charge Payable (both liabilities), never revenue.
- **Sync log** (`_shared/syncRun.ts`): one row per day or bill, updated in place, a compare and set lease so two runs never post at once, 'sending' then 'posted' per transaction, Xero Idempotency-Key, a lost answer looked up by Reference, history of every attempt. No schema change.
...
## ADR-027: US sales tax is charged on the bill's own basis (v5.9.30)

**Context:** found 19 Sep 2026 on main: every surface charged US added-on (exclusive) sales tax on item price x qty BEFORE any discount, and never on the service charge or delivery fee (v5.7.31 kept that basis deliberately to match what the tax lines already booked). In most US states a store discount reduces the taxable amount, so every discounted US check over-collected; a mandatory service charge is taxable in New York, California and most states, so those checks under-collected. Only US venue: The Cabin, 0 checks, so nobody was charged wrongly. UK inclusive VAT had to stay byte identical.

**Decision:**
- **One seam, one allocation.** `computeOrderTaxUnified` takes an optional `checkBasis` `{ discounts, service, deliveryFee }`; `lib/taxBasis.js allocateCheckBasis` gives each line its value after its own item discount and its share of every check discount (a discount that names its units, auto discount `appliedItems`, goes to those lines first; the rest pro rata by value), plus its share of the service charge and delivery fee (pro rata by discounted value, so an exempt line's share stays untaxed). The engine applies each line's profile settings.
- **Per tax line settings, US defaults** (`taxEngine.lineBasisSettings`, the one place the defaults live): `tax_basis` post_discount, `tax_service_charge` ON, `tax_delivery_fee` OFF (state rules differ, the operator switches it on). Legacy `tax_rates` rows: exclusive rates take the same defaults in the adapter. An explicit value always wins. Migration 20260919t adds the two columns and moves existing exclusive rate lines (all at The Cabin) to post_discount.
- **Store discounts:** item, check, auto, promo code and loyalty reward all reduce the taxed amount. Gift cards and booking prepayments are tenders, not discounts: they never change the tax. Promo and loyalty credits are taken off after the bill at checkout, so the checkout recomputes the tax with them (`taxFor`, `creditDiscounts`) and takes the relief off; the split bill keeps the gross bill (it carries no credits).
- **Each surface taxes what it charges.** MPOS charges lines after item discounts only (it ignores check discounts and service, a separate known gap), so its basis is exactly that (`LINES_ONLY_BASIS`) and the close books the tax it charged (`chargedTaxBreakdown`). Channel orders (HubRise, ezCater) are unchanged: the marketplace sets and remits their tax.
- **Records:** the tax booked is the tax charged. POS closes recompute through `computeCheckTotals` with the checkout's credits (the terminal draft carries `taxCredits` for the reconciler). Kiosk, online, QR and catering write `tax_breakdown` when added-on tax was charged. Reports and reprints read the stored breakdown for such checks (`recordedCheckTax`); everything else recomputes as before.
- **Refunds:** a part refund gives back the added-on tax those items' goods carried (the stored per line `lineTaxes`, goods only, else pro rata by value) plus the service charge tax in proportion to the service refunded (`serviceTax`; kept service keeps its tax); delivery tax goes back only on a full refund. A full refund returns what is left, tax included. The refund entry records `addedTaxAmount`. A per-unit levy (sugar levy) is added on top too, so it goes back with its items (no UK venue has a levy today).
- **Review round 1 (19 Sep 2026), 4 US bugs fixed:** kiosk V2 sized the gift card before the tax relief (over-debit); realtime and force sync dropped the booked breakdown (US refunds on another till returned no tax); online fell back to pro rata scaling when an offer hit only an exempt line (under-charged); a part refund returned service tax on a kept service charge. Also: a 0% exclusive row no longer leaves the parity path; catering's delivery fee (booked in `service`) is not counted twice on a recompute; the migration's basis move is guarded by `tax_service_charge is null`.
- **Release order:** run migration 20260919t BEFORE the web deploy. Until it runs, The Cabin's generated 'Sales Tax' profile still says pre_discount, so it is not a mirror and its lines tax the menu price while legacy-rate lines tax the discounted price.
- **UK lock:** inclusive lines never read the basis. Proven: 2000 random checks in `taxBasis.test.js`, and 9000 random checks compared against origin/main (totals, booked breakdown, refunds) before release.

**Consequences:** US check totals change whenever a discount, service charge or delivery fee applies (lower on discounts, higher on a taxable service charge). The kiosk card path fingerprints moved (owner sign off + kiosk hardware test). Known gaps, not changed here: MPOS charges ignore check discounts and service charge and treat an amount item discount as a percent (all venues; the MPOS tax now uses the correct amount discount, so on such a line the goods and the tax disagree until MPOS is fixed); a POS walk-in record stores `service: 0` even when a counter dine-in service charge was charged (its tax is booked); a partial refund returns the item's menu price, not its discounted price (all venues); UK VAT on the till is still worked out on the pre-discount price (online has scaled it by offers since v5.5.787) and never on a compulsory service charge (byte identical was required).
