...
**Decision:**
- "Sync ezCater menu" (staff only, Item matching card) and a daily sync (pg_cron hourly, a venue is due after 20 hours without a good sync) read the CURRENT menus (venue date) of every caterer mapped to the venue and write `ezcater_item_links` rows with their published ids (`ez_ids`): one row per plain item, one row per SIZE of a multi size item (key `<item>|size:<size>`, `ez_size_name` set), one row per option value when ezCater lets us read values. New rows are not yet ordered (`seen_count` 0). A sync never deletes a row, never changes a staff decision, "Not on our menu" or an exact auto link, and never touches `seen_count`; a second sync inserts nothing. The one decision a sync does revisit is an auto link the ORDER TIME name rule made (`matched_by 'name'`) on a row it now syncs: with our whole menu in hand it is decided again by the exact rule (kept and marked `exact`, replaced by the exact match, or cleared for staff), guarded so a person who saved first wins. A single size item's one size is stored on its plain row as `ez_only_size` (display only, never in the key), so the Item matching card shows "Turkey Sandwich, sold only as Box" and suggests on item plus size: staff never match blind. The refresh names only the ezCater fact columns; `source` got a default of `'auto'` in 20260919m because Postgres builds the full insert tuple before ON CONFLICT DO UPDATE, and without a default every refresh of an existing row failed on the NOT NULL `source`.
- Auto links: EXACT means exact. A sync's auto link is written with `matched_by 'exact'`; the order time name rule writes `'name'`. The full ezCater name (item plus its size) must equal exactly one of our item names after only case, accents, whitespace and punctuation are folded (`exactName`); nothing else is dropped, so any size or container word ("Large", "Tray", "Box", "Serves 10") on one side only means no auto link. A single size item's only size takes part like a multi size item's sizes do; the one allowance is a size name whose every word the item name already says ("Italian Boxed Lunch" with its only size "Box", Box folded with Boxed), which adds nothing. So "Turkey Sandwich" sold only as a Box is not our plain "Turkey Sandwich": a Box and a Tray of the same food are different products. A size row also needs its size to be its own (not shared with a sibling). Options: exactly one of our options with the same exact name. Everything else waits for staff with suggestions.
- Order time, EXACT MEANS EXACT here too (review round 2): when the line's published size id is on a synced row (a size row OR a single size plain row), or a line with no size finds a synced row by its name key, ONLY that row's decision counts: a staff match, or an exact auto link (`matched_by 'exact'`). A row with no such decision leaves the line unmatched (it prints by name). The order time name rule (which drops size words to find candidates) never decides such a line and never fills or upgrades a row that has `ez_ids` or `synced_at`; only its seen count moves. Item codes and posItemId are not consulted for a line on a synced row either. ANY other sized line stays unmatched. A line with no synced row and no size keeps the rules from before. Before migration 20260919m runs, nothing changes.
...
Migration: `20260919m_OPS_ezcater_menu_sync_v1.sql` (renamed: its first name clashed with a parked branch), run AFTER ezcater-connect and ezcater-webhook are deployed, then press Sync.
