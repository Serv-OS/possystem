...
**Context:** Peter (18 Sep 2026) wants ezCater items matched BEFORE any order, not pasted by hand and not left to ezCater's menu team. The connected token can read menus (proven live, read only). The full sync on `feat/ezcater-menu-sync` routed wrong sizes and wrong items in review. Peter chose the simpler version. Three review rounds of this version kept finding loose name matches at order time, so review round 3 (18 Sep 2026) removed the loose paths instead of patching them.

**The rule, in plain words:** after the sync is set up, orders only use matches made before the order: exact names found by the sync, or matches staff saved. There is no name matching at order time. Anything else prints by name.

**Decision:**
- The sync. "Sync ezCater menu" (staff only, Item matching card) and a daily sync (pg_cron hourly, a venue is due after 20 hours without a good sync) read the CURRENT menus (venue date) of every caterer mapped to the venue and write `ezcater_item_links` rows with their published ids (`ez_ids`): one row per plain item, one row per SIZE of a multi size item (key `<item>|size:<size>`, `ez_size_name` set), one row per option value when ezCater lets us read values. A single size item's one size is stored on its plain row as `ez_only_size` (display only, never in the key), so the card shows "Turkey Sandwich, sold only as Box": staff never match blind. A sync never deletes a row and never touches `seen_count`; a second sync inserts nothing.
- Exact means exact (`exactName`: case, accents, spacing and punctuation folded, nothing else dropped). An item auto links (`matched_by 'exact'`) only when its full name equals exactly one of our item names: the item plus its size, where a single size item's only size counts unless every word of it is already in the item name ("Italian Boxed Lunch" sold as "Box"). So "Turkey Sandwich" sold only as a Box is not our plain "Turkey Sandwich". A size row also needs its size to be its own (not shared with a sibling). An option auto links only when the group AND the value are both exact and exactly one of our options matches (review round 3): "Bread: White" is never our "White" in the Cheese group.
- Every sync decides every automatic row again (review round 3), not only the old order time name links (`matched_by 'name'`): with our whole menu in hand a row gets the one exact match (kept, moved to it, or set) or is cleared, guarded on the values read, so a person who saved first wins. This covers ezCater changing a size (Caesar Salad sold only as Regular, then only as Large: our "Caesar Salad Large" or nothing, never Regular), an unsized item becoming sized (Turkey Sandwich sold only as Box, Italian sold only as Small), a rename under the same key (Sandwich Platter to Sandwich Platter Tray) and our own item being renamed. An automatic row the read did not cover (off the current menus) keeps its link only while its stored name is still exactly the one item of ours it points at. With our menu read only in part nothing new is linked, and an exact link whose ezCater name changed is cleared.
- Staff decisions (a match, "Not on our menu", a clear) are never changed by a sync. `decided_as` (20260919m) records the full name the staff saw when they saved it: items_save takes it from what the card showed, and for a decision made before 20260919m the first sync records the name the row showed then. When ezCater changes that name or size afterwards, the card asks staff to look again ("Still right" or "Change"), and until they do, orders do not use that match: those lines print by name. A "Caesar Salad" matched as Regular must not route the Large ezCater now sells under that name.
- Published ids are only ever ADDED while a row's full name stays the same (new first, old kept), so a change to an order placed before a republish still matches. When a row's full name changes (a new only size, a rename under the same key), the old ids belonged to a different product and are dropped: a change to an old Regular order prints by name rather than routing as Large.
- Order time, after 20260919m (`planSyncedLineMatches`): a line matches ONLY when its published size id is on a synced row with a trusted decision: a staff match not waiting for a look again, or an exact auto link from a sync. The order's `menuItemSizeId` IS the menu's `sizes.id` (proven on HKX77V, 18 Sep 2026; every HKX77V line carries one); the order's item id is never the menu's item id and is never matched. A customization matches only by its published id (`customizationId`, not yet seen on a live order: if it is not the menu's value id, no customization ever matches and each prints by name, never a wrong one). No name matching at order time at all: not `autoLinkDecision`, not the saved name links, not the legacy key fallback, not a synced row found by its name, not a posItemId, not an item code. Every other line, sized or not, prints by name, and an order writes nothing but the seen counters of the synced rows its ids landed on. The card stops listing rows no sync wrote (orders never use them) and stops offering item codes for ezCater.
- Before migration 20260919m runs, the order time rules are exactly the ones on main (`planNameMatches` is main's `planLineMatches`, unchanged).
- A failed or partial link read never falls back to name guessing, for ANY line: only a PROVEN missing sync column (or missing links table) gives the old rules; any other failed or cut short read sends the order through exactly as ezCater sent it (every line prints by name) and writes nothing.
- Writes: what staff saw first, then the automatic decisions, then inserts, then refreshes. A row whose baseline or decision write fails is not refreshed, so new ids never land next to an old decision. The refresh names only the ezCater fact columns; `source` got a default of `'auto'` in 20260919m because Postgres builds the full insert tuple before ON CONFLICT DO UPDATE, and without a default every refresh of an existing row failed on the NOT NULL `source`.
...
**Consequences:** from the moment 20260919m runs until the first sync, every ezCater line prints by name, so the release note runs the migration and the sync together, outside service. After ezCater republishes a menu, NEW orders print by name until the next sync; staff can press Sync. Fewer rows auto link than a looser rule would (for example "Farmhouse Salad" sold only as "Serves 1" waits for staff); that is the price of never linking the wrong size. Item codes and posItemId no longer route ezCater orders once the sync is set up (ezCater confirmed posItemId stays null for this integration).

**Known limit (not fixed):** two ezCater items whose names differ only by a trailing tray or pan word ("Sandwich Platter" and "Sandwich Platter Tray") share one key row, because the key drops a trailing container word. The sync marks that row as described two ways and refuses to auto link it, so only a staff match can join them, and that one match then routes both. Telling them apart needs a key change, which would orphan every saved match; not worth it for v1.

Migration: `20260919m_OPS_ezcater_menu_sync_v1.sql` (renamed: its first name clashed with a parked branch; `decided_as` added in review round 3), run AFTER ezcater-connect and ezcater-webhook are deployed, outside service, then press Sync.
