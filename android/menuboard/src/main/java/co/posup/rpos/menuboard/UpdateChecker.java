...
/**
 * In-app auto-updater for the sideloaded Menu Board APK (TV / Fire TV; no Play Store).
 *
 * On launch / resume (throttled) it reads latest-menuboard.json from Supabase Storage; if its
 * versionCode is higher it downloads the new APK via DownloadManager and launches the system
 * installer (one tap: "Install"). First run per device needs a one-time "allow install from this
 * source" grant (Android 8+ / Fire OS). In-place install only succeeds when every build is signed
 * with the SAME key — the Menu Board CI builds a fixed-key RELEASE APK for this reason.
 */
public class UpdateChecker {
    private static final String MANIFEST_URL =
        "https://tbetcegmszzotrwdtqhi.supabase.co/storage/v1/object/public/app-releases/latest-menuboard.json";
    private static final long THROTTLE_MS = 3L * 60 * 60 * 1000; // re-check at most every 3h
    private static final String PREFS = "menuboard_update";
    private static final String APK_NAME = "menuboard-update.apk";
    ...
    private void beginUpdate(String apkUrl, String vName, boolean mandatory) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O
            ...
            AlertDialog.Builder b = new AlertDialog.Builder(activity)
                ...
                .setMessage("To keep the menu board up to date automatically, allow it to install "
                          + "app updates. Open Settings, turn on \"Allow from this source\", "
    ...
    private void download(String apkUrl, String vName) {
        try {
            ...
            req.setTitle("Serv OS Menu update" + (vName.isEmpty() ? "" : " " + vName));
