...
/**
 * In-app auto-updater for the sideloaded Serv OS device apps built on :webshell (no Play Store).
 *
 * Flow (auto-download + one-tap install):
 *   on launch / resume (throttled)
 *     -> fetch latest-<channel>.json from Supabase Storage
 ...
 * key (see android/RELEASING.md). The device apps CI (build-device-apps.yml) builds a fixed-key RELEASE APK for exactly
 * this reason.
 *
 * latest-<channel>.json shape (hosted at MANIFEST_URL):
 *   { "versionCode": 2, "versionName": "1.1",
 *     "apkUrl": "https://.../storage/v1/object/public/app-releases/<channel>.apk",
 *     "mandatory": false, "notes": "what changed" }
 */
public class ShellUpdateChecker {
    // The shared webshell copy of the MPOS UpdateChecker: the same flow, with the update channel and
    // the app name passed in (latest-<channel>.json, <channel>-update.apk).
    private static final String RELEASES =
        "https://tbetcegmszzotrwdtqhi.supabase.co/storage/v1/object/public/app-releases/";
    ...
    public ShellUpdateChecker(Activity activity, String channel, String appName) {
        this.activity = activity;
        this.MANIFEST_URL = RELEASES + "latest-" + channel + ".json";
        this.PREFS = channel + "_update";
        this.APK_NAME = channel + "-update.apk";
    ...
    private void beginUpdate(String apkUrl, String vName, boolean mandatory) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O
            ...
            AlertDialog.Builder b = new AlertDialog.Builder(activity)
                ...
                .setMessage("To keep " + appName + " up to date automatically, allow it to install "
    ...
    private void download(String apkUrl, String vName) {
        try {
            ...
            req.setTitle(appName + " update" + (vName.isEmpty() ? "" : " " + vName));
