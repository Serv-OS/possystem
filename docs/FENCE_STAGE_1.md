...
- **Only what your storefront really sells is priced** (fix round 4): a line that is a size's parent row, an option only sub item, an archived item, an item you have hidden from online, an item that is **86'd**, or an item with no price on your menu is one the server cannot value. It still goes to the kitchen, under your menu's own name, but the order arrives **Payment short** (or **Payment being checked**) for staff to confirm, and it is never free. Before this round those lines were worth nothing and rode along on a paid ticket.
