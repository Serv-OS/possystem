...
5. **`payment-proof` (deploy it again even if it is already live)** and, if not deployed yet, `location-admin`. Fix round 4: `payment-proof` now records what a loyalty reward IS (a free item and the items it covers, or a percent), which is how the server values it after step 3. Without this deploy every free item and percent redemption arrives **Payment short** for staff to confirm.
...
- **One loyalty reward per order** (fix round 5): loyalty rewards used to be added together with no ceiling, so two genuine rewards redeemed against one bill (the storefront only ever applies one, but the page can be crafted) took a 125 pound order to 0.00 and it booked as **paid with no money at all**. Now the single dearest reward the server can value counts, a percent reward is that percent of what is **left** to pay after your own automatic deals and any promo code, and the whole loyalty discount can never be more than that figure. An order that claims more than that arrives **Payment short** for staff.
- **A QR tab's tip can only be money taken above the bill** (fix round 5): the tip was the last money field on a QR check still decided by the phone. A tab of one 95 pound meal could be closed declaring a 95 pound tip: the venue booked a 0.00 sale and then paid 95 pounds of its own takings out through tronc and the P&L. The tip is now capped at whatever the card took **over** the server's own value of the rounds; the rest is booked as the sale it is.
...
- **Prices** (fix round 2, tightened in fix round 4): the server now prices every online, QR and catering order from the menu and proves every discount, so the food itself can no longer be underpaid. Still what the page says: delivery fees, service charges and US sales tax (a doctored page can leave those off), tips (the customer's choice), and a promo code's per customer limit (the server has no customer identity). The automatic deals the server works out follow the storefront's rules exactly; two deals of the same priority on the same items may be applied in a different order and show "Payment short" (staff confirm).
- **Loyalty rewards** (fix round 4): a reward is now worth only what the server can work out it is worth. A fixed amount reward is worth its amount, a percent reward that percent of the server's own goods total, and a **free item** reward the cheapest matching item on that order (matched by name as well as by id, because loyalty is per company and menu ids are per site). Anything else is worth **nothing** and the order arrives **Payment short** for staff to confirm: a reward redeemed too slowly for its proof to be written, a free item reward for something that is not on the order, or a reward set up in a shape the server does not know. Until this round a reward with no money value of its own (a free item, which is what stamp cards give, or a percent) was worth the **dearest single item on the basket** and stacked once per redemption: one genuine free coffee took 95 pounds off, and two took 190.
