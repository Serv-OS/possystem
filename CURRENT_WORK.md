...
> **Current build: v5.5.698** · live: https://possystem-liard.vercel.app · dev: https://dev.serv-os.app · repo: **Serv-OS/possystem** (branch `develop`, Vercel auto-deploys).
...
- **⏳ Kitchen batch cooks — REMAINING (needs owner sign-off).** `production_batches` records what was COOKED, not a SCHEDULE — so the mockup's scheduled prep (due time / overdue / "Record") needs a NEW additive `prep_schedule` table (recurring template) + a daily completion link + a BO editor + a record write path (edge action like manager-approve). Migration = Peter's per-change authorization. `kitchen.js` `groupBatches`/`batchStatus` already model the batch shape.
