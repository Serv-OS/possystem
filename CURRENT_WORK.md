...
- 9 Sep: FIRST LIVE PAYMENT (£3, OL-BVUIH, psp GDWP6G59K4B528Q9) on FranPOS_QSR_UK with no store. v5.8.41 guided go live flow (AdyenGoLiveFlow + golive_state), v5.8.42 flow shows Adyen refusals + audit rows, v5.8.43 real /paymentMethods (Apple Pay + Google Pay can appear), order_queue.paid stamped, balance platform id remembered (migration 20260908c APPLIED), v5.8.44 link_store + one plain reason on a refused BP read. Balance Platform key set (ADYEN_LIVE_UK_BP_KEY, ADYEN_LIVE_UK_BALANCE_PLATFORM; "FranPOS_UK" is the balance platform id). Store ST32DDL22322BQ5PXJVN95JSM saved on Provo; holder AH32BZP22322CJ5PXF2BD5FTR read OK (not yet written on the row); no split, no sweep (commission not collected). Live AMS1-000168254080883 registered 22:29.
- IN FLIGHT: wf_2a8d676b (link_holder + Payouts and commission step). Review fixes applied 9 Sep (working tree, NOT committed, NOT deployed): pasted holder wins over a store's foreign balance account (never adopted, step 5a says so); set_split writes ONE RULE PER TIER from resolveAdyenRateCard, checks the balance account belongs to the row's holder, reads the store's merchant first, writes the row only after Adyen accepted, caps at 10% or 100p without confirm (screen asks for typed CONFIRM past 3% or 50p); step 5 offers nothing while the flow looks at the other environment and the server refuses a wrong body.environment; one sweep only (retarget, idempotency key names the bank, bank chosen from the holder's approved list); old split profiles are never deleted; payouts_ok is the CAPABILITY again and payout_sweep_id (NEW column, migration 20260909b_PLATFORM_adyen_payout_sweep_id.sql, PENDING Peter) is PAID OUT, the list chip reads both; request_payouts action; needs_details state. Redeploy adyen-terminal-admin, adyen-onboard, adyen-bp-webhook, payments-admin. QUEUED: Card readers page rebuild (one venue list, rename, Add by serial, storeIds scoped inventory, Advanced collapsed). Peter's rule: the whole reader experience must be understandable by a venue owner.

- 9 Sep late (v5.8.46): LIVE READER DIAGNOSIS from Adyen's own records (adyen-terminal-charge action diag_reader, service role only, read only). Live key = credential ws_570369@Company.FranPOS "ServOS live", 89 roles incl. Cloud Device API role, restricted to FranPOS_QSR_UK. AMS1-000168254080883 is boarded on that merchant + store ST32DDL (SV-1007), cloud connected (lastActivityAt minutes old, /connectedTerminals lists it on EU, US, AU, APSE hosts). Same key: /sync and /async answer 403 "010 Not allowed" on ALL FOUR regional hosts, device-api 00_403. A STANDALONE payment on the reader itself works and lands in the CA. The £3 online payment worked with the same key. TEST credential (22 roles, no Cloud Device API role) passes the test gate. So: not roles, not region, not the terminal, not the store; the live cloud Terminal API gate refuses this credential. Pay at table missing on the reader = the live STORE holds no wake up button (nexo.notification disabled) and no event URL; the test store has both (never pushed to live). Live secrets: no separate MGMT key (managementKey = apiKey); ADYEN_LIVE_UK_BP_KEY is a Balance Platform key (401 on management and terminal hosts, as expected).
- 10 Sep: ROOT CAUSE (22 agent adversarial review, Google Doc "ServOS live reader, why Adyen refuses payments"): the live credential ws_570369 carries the role "Thirdparty interaction with in-store terminals" (every role was ticked). Adyen docs: a third party credential cannot send payment requests to terminals, "refused with an HTTP status code of 010 Not Allowed", and that role must be the ONLY role on a dedicated thirdParty_ credential. Test credential lacks it. FIX = untick that role on the live credential (no key regeneration), then re run diag_reader. Second candidate: per environment "Enable terminal API" toggle (Devices > Device settings > Integrations). v5.8.47: SaleToAcquirerData store= now sends the Adyen store REFERENCE (SV-1007) read from GET /stores/{id}, not the ST id (403 910 Invalid Store on live; equal on test).
- 10 Sep 00:55 UTC CONFIRMED: Peter unticked "Thirdparty interaction with in-store terminals" on ws_570369; within a minute /sync Diagnosis answered 200 on every host (reader: TerminalApiEnabled true, WebsocketConnected true, storeId SV-1007). Nothing else changed. Rule: never select all roles on an Adyen credential.
- 10 Sep NIGHT BUILD (v5.8.48, Peter asleep, his brief: no pasting ids, real per card type rates not a flat commission, Processing and readers pages "a complete mess"): spec in the session scratchpad night_spec.md; two worktree builders + a 5 lens adversarial review. Step 5 = "Card rates and payouts" (four rates table, Apply these rates on Adyen = set_split with NO numbers, six rules one per tier, 0/0 tier allowed, legacy markup_* never written, ratesOnAdyen compares Adyen's rules to the resolved card); zero paste: set_balance_platform (once per env/region, validated by GET /balancePlatforms/{id}), link_all (store + holder saved in one click), paste only under Advanced; Processing page reordered (flow, Card rates via shared RateCardRows.jsx, Plan, Advanced); readers page rewritten (AdyenTerminals.jsx: one list, rename, add by serial, storeIds scoped list, venue tips, Advanced) with NEW actions sync_store_settings (BO user OR service role; event URL + Pay at table button + payAtTable + gratuities, saved on locations.pos_settings.reader_tips / reader_settings_sync) and rename; assign calls sync automatically; unlink retires the platform row. Provo's live store received the settings at 04:50 UTC (verified by diag_reader). Helpers: src/lib/payments/rateCard.js, readerSettings.js (+ TS mirror).
- 10 Sep review of the night build: 51 findings, 42 confirmed and fixed before shipping (money: rates read failure never applied, 5% / 50p limit asks again, profileMatchesRules compares Adyen's whole rule set, explicit zero commission block, stale editor refused; onboarding: holder ambiguity picker, platform name input on the right step, no paste wording on the main path; readers: existing tips kept, tip on till payments kept, messages next to the button). NOT yet proven against Adyen: the explicit zero commission block (Provo has no zero tier). Pending Peter: Platform migrations 20260909 (liable balance account) and 20260909b (payout_sweep_id); press Apply these rates on Adyen for Provo after checking in person 0.8% + 5p, online 2% + 10p, Amex 2.75% + 15p, keyed 2.5% + 10p.
- 10 Sep 14:29 UTC FIRST LIVE SPLIT CHECKED (adyen-terminal-charge diag_payment, service role, read only): Provo £1.00 Visa Debit contactless, psp FSPKMNZ492CWX7Z3, catch all rule 80 bps + 5. Adyen transfers: venue balance account +94p (ref HR2JC7RZMQ6ZCWW3), platform liable account BA32DFM22322CC5PTXKTJ7DBX +5p fixed (FP...) and +1p percent (PP..., 0.8p rounded up), PaymentFee 1p out of the liable account (WQ...). Split profile SCNF42BX6223225W5PXN9ZJ3TW4T7H rules match the rate card exactly. Ops migrations 20260908b guard + 20260909 cron APPLIED 10 Sep (adyen-unsent-sweep-1min answers 200); Platform 20260909 + 20260909b APPLIED. OPEN QUESTION for Peter: payments-admin reseller statement net_due = commission minus (0.10% + 5) and does NOT subtract actual interchange and scheme fees, while FRANPOS_RESELLER_AGREEMENT.md clause 5.1 says (Merchant Fees less Actual Interchange) less Buy Rate Margin. Not changed. v5.8.50 Pay at table switch back per reader (modes.table_pay).
- 10 Sep v5.8.52: admin Revenue page 'Check a payment' (PaymentCheck.jsx; adyen-terminal-admin payment_list + payment_breakdown, super_admin or service role, read only; pure helpers paymentBreakdown.js and resellerRate.js with TS mirrors). FranPOS fixed fee per currency: platform_settings rate history 2026-08 entry now carries fixed_minor_by_currency GBP 3, USD 5, EUR 5 (Peter: 5 US cents = 3p). Review: 40 findings, 33 confirmed, 26 fixed. DECISIONS WAITING ON PETER: (1) the FranPOS statement does not take Adyen fees off, so it says 3p where the payment check says 2p; (2) the statement counts test card payments (live not true); (3) the draft agreement still says 5 minor units, which means 5p for GBP.
- 10 Sep v5.8.53 BOOKING PAYMENT GATE + GUEST PRE ORDER CHOICES (workflow: 2 builders, 39 review findings, 33 confirmed and fixed; 1065 tests). Server only confirms after Adyen authorises the full due (booking_pay Authorised only, 3DS via booking_pay_details, booking_status poll, stored due on the booking, webhook amount checks, needs_refund + urgent activity alert). Packages needing payment hidden online when capture off or Adyen unusable. Till tables and host stand label Awaiting payment; No show hidden on unpaid; Undo no show never confirms unpaid. Guest pre order choices via OnlineItemSheet, carried to host stand, till line (price includes mods, stock at seat) and kitchen ticket; floor card double count fixed. Integrator change: an authorised hold confirms WITHOUT a saved card token (0 of 875 live Adyen events in 30 days carried one; the fixer's token rule would have expired every Provo hold booking); both token keys read. DEPLOYED booking-widget + adyen-webhook. MIGRATION PENDING (Peter): supabase/migrations/20260910_OPS_booking_payment_gate_and_preorder_choices.sql (payment columns, needs_refund status, promote_paid_booking RPC, sweep skips young pending payments, bookings_payment_gate trigger, booking_preorders mods/variant columns). OPEN: double charge if an abandoned 3DS later authorises after a second card (flagged needs_refund, no auto refund); needs_refund refunds are manual in Adyen; server keeps only known options but does not enforce min/max; whether a card hold counts as paid is Peter's call; no live 3DS or webhook test run yet. Google bookings plan: Google Doc 'ServOS bookings on Google, Apple and Instagram (10 Sep 2026)'.
- 11 Sep v5.8.54 VENUE CARD PAYMENTS FEES: Back Office ServOS Payments Fee column and tiles showed adyen_payments.fee_minor (Adyen cost from settlement reports, never received on live) so live fees read as a dash. Now venue facing screens show the venue fee (commission_minor, reduced by refund ratio) and You receive; no venue facing action returns Adyen cost or margin (payments, payouts, payout_detail, statement). Live venue hides test payments (switch to show). Venue time zone on screen and CSV. Helpers src/lib/payments/venueFees.js (+ _shared/venueFees.ts parity) and venueTime.js. Review 26 findings, 14 confirmed, 14 fixed (uncaptured holds not counted, holds captured below their hold restamp amount and fee in adyen-webhook, paged reads past 1000 rows, payouts live fence). Proven on Provo: 6 live payments, £9.50, fees £0.47, venue receives £9.03. DEPLOYED adyen-financial + adyen-webhook. OPEN: disputes tab not live filtered; live settlement report still not set up (admin margin only).
- Supabase token trap: ~/.zshrc line 5 exports an OLD sbp_ token that overrides the current one on ~/.zshenv line 4; commands must export the .zshenv one explicitly (grep -o 'sbp_[A-Za-z0-9_]*' ~/.zshenv | tail -1).
