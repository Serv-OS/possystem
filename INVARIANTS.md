...
- **The boot re-link uses the device secret, never a code read back from the table.** Codes are server made, 60 minutes, single use after file 2. The one time `device_secret` lives in `rpos-device.deviceSecret` (tills) or `rpos-kiosk-secret` (kiosks, in the tenant fence keep set).
