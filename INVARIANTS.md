...
- **Added-on (US) sales tax is charged on the bill's own basis, in ONE place (v5.9.12).** Every surface hands `computeOrderTaxUnified(items, taxCtx, orderType, checkBasis)` the discounts, service charge and delivery fee IT charges (`checkBasis` = `{ discounts, service, deliveryFee }`; promo and loyalty credits go in as discounts via `taxBasis.creditDiscounts`). `lib/taxBasis.js` allocates, each profile line decides (`taxEngine.lineBasisSettings`: post_discount, service ON, delivery OFF by default). Never compute added-on tax from items alone on a surface that has discounts, service or delivery. A close BOOKS the tax the bill CHARGED: POS closes recompute through `computeCheckTotals` with `creditDiscountsFromPayment(paymentInfo)`, MPOS hands over `chargedTaxBreakdown`, customer surfaces write the breakdown they charged. Tips never enter the basis.
- **Inclusive VAT never reads the check basis.** Inclusive lines extract from the shelf price exactly as before, so a UK check is byte-identical with or without a basis (taxBasis.test.js UK LOCK fuzz). The new result keys (`lineTaxes`, `serviceTax`, `deliveryTax`, `checkBasisApplied`) and the `tax_breakdown` column on customer surfaces appear ONLY when added-on tax was charged; do not add them to inclusive-only records.
