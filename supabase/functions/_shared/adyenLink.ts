...
export interface GoliveStateInput {
  ...
  // { needsPlatform, ambiguous, capped }: what the BUSINESS ACCOUNT search came
  // back with when no holder was resolved (10 Sep 2026), so steps 1 and 2
  // never say "no business account" for a search that did not run or split.
  holderRead?: { needsPlatform?: unknown; ambiguous?: unknown; capped?: unknown } | null;
  ...
  // readFailed: the venue rates could not be read, so the tiers are a fallback.
  rates?: { currency?: unknown; tiers?: unknown; priced?: unknown; unpriced?: unknown; onAdyen?: unknown; liableBalanceAccountId?: unknown; readFailed?: unknown } | null;
  commission?: unknown;
...
export const DEBIT_TIER_BASE: Readonly<Record<string, string>> = Object.freeze({ card_present_debit: 'card_present', card_not_present_debit: 'card_not_present' });
export const DEBIT_TIERS: readonly string[] = Object.freeze(Object.keys(DEBIT_TIER_BASE));
// Every row of the rate card, in the order every screen lists them.
export const RATE_CARD_TIER_ORDER: readonly string[] = Object.freeze(['card_present', 'card_present_debit', 'card_not_present', 'card_not_present_debit', 'amex', 'keyed']);
const DEBIT_RULE_INTERACTION: Readonly<Record<string, string>> = Object.freeze({ card_present_debit: 'POS', card_not_present_debit: 'Ecommerce' });
...
// five names stamp the ledger) and src/lib/payments/adyenLink.js.
...
export interface FundingPolicy { prepaid: string; deferredDebit: string; charged: string; keyedDebit: string; businessDebit: string }
export const FUNDING_POLICY: Readonly<FundingPolicy> = Object.freeze({
...
export function debitFundingSources(policy: unknown = FUNDING_POLICY): string[] {
  const p: Dict = isObj(policy) ? policy : FUNDING_POLICY;
  return [
    'debit',
...
function samePrice(a: unknown, b: unknown): boolean {
  const x: Dict = isObj(a) ? a : {};
  const y: Dict = isObj(b) ? b : {};
...
function debitOwnPrice(tiers: unknown, debitTier: string): Dict | null {
...
function debitEffective(tiers: unknown, debitTier: string): Dict {
...
export function debitTiersApart(tiers: unknown): string[] {
...
export function tieredCommissionRules(currency: unknown, tiers: unknown, policy: unknown = FUNDING_POLICY): { rules: SplitRule[]; lacking: string[] } {
  const t: Dict = isObj(tiers) ? tiers : {};
  const p: Dict = isObj(policy) ? policy : FUNDING_POLICY;
  const commissionFor = (c: Dict): AdyenCommission => commissionFromRates(c.percent, c.fixedPence ?? c.fixed_pence) ?? { variablePercentage: 0 };
  const commissionOf = (tier: string): AdyenCommission => commissionFor(isObj(t[tier]) ? t[tier] : {});
...
export interface RateTier { percent: number | null; fixedPence: number | null; source?: 'venue' | 'platform default' | null; inheritedFrom?: string | null }
...
export const RATE_TIER_LABELS: Readonly<Record<string, string>> = Object.freeze({ card_present: 'In person', card_not_present: 'Online', amex: 'Amex', keyed: 'Keyed', card_present_debit: 'In person debit', card_not_present_debit: 'Online debit' });
...
export function tiersFromResolved(cards: unknown): Record<string, RateTier> {
  ...
  const sourceWord = (c: Dict): RateTier['source'] => {
...
export const RATE_ROW_LABELS: Readonly<Record<string, string>> = Object.freeze({
...
export function tierRowList(tiers: unknown): string {
...
export interface RateCardProblem { tier: string; field?: string; text: string }
export function rateCardProblems(card: unknown, { currency, verb = null }: { currency?: unknown; verb?: unknown } = {}): { errors: RateCardProblem[]; overLimit: RateCardProblem[] } {
  const c: Dict = isObj(card) ? card : {};
  const errors: RateCardProblem[] = [];
  const overLimit: RateCardProblem[] = [];
  ...
  for (const tier of RATE_CARD_TIER_ORDER) {
    const row: Dict = isObj(c[tier]) ? c[tier] : {};
...
const SPLIT_LOGIC_KEYS: readonly string[] = Object.freeze(['paymentFee', 'remainder', 'tip', 'surcharge', 'chargeback', 'chargebackCostAllocation', 'refund', 'refundCostAllocation']);
const OTHER_SPLIT_KEYS: readonly string[] = Object.freeze(['acquiringFees', 'adyenFees', 'adyenCommission', 'adyenMarkup', 'interchange', 'schemeFee', 'additionalCommission']);
function splitRuleKey(r: Dict): string {
  const cond = (v: unknown): string => (!str(v) || lower(v) === 'any' ? 'any' : lower(v));
...
function sameSplitLogic(a: unknown, b: unknown): boolean {
  const x: Dict = isObj(a) ? a : {};
  const y: Dict = isObj(b) ? b : {};
  const cx: Dict = isObj(x.commission) ? x.commission : {};
  const cy: Dict = isObj(y.commission) ? y.commission : {};
  const whole = (v: unknown): number => { const n = Number(v); return Number.isFinite(n) ? Math.round(n) : 0; };
  ...
  const present = (v: unknown): boolean => v !== undefined && v !== null && v !== '' && !(isObj(v) && !Object.keys(v).length);
  return !OTHER_SPLIT_KEYS.some((k) => present(x[k]) !== present(y[k]));
}
export function profileMatchesRules(profile: unknown, expectedRules: unknown): boolean {
  const actual: Dict[] = (Array.isArray((profile as Dict)?.rules) ? ((profile as Dict).rules as unknown[]) : []).filter(isObj);
  const expected: Dict[] = (Array.isArray(expectedRules) ? expectedRules : []).filter(isObj);
...
export function ratesOnAdyen(profile: unknown, venueTiers: unknown, currency: unknown = 'GBP'): RatesOnAdyen {
...
// KEEP IN SYNC with src/lib/payments/adyenLink.js (adyenLink.test.js).
export interface RatesChangeRow { tier: string; label: string; now: string | null; next: string | null; changed: boolean }
export interface RatesChangePreview { same: boolean; canSend: boolean; rulesNow: number; rulesNext: number; rows: RatesChangeRow[]; lines: string[] }
export function ratesChangePreview(profileNow: unknown, venueTiers: unknown, currency: unknown = 'GBP'): RatesChangePreview {
  ...
  const priceLine = (tiers: unknown, tier: string): string | null => {
  ...
  const rows: RatesChangeRow[] = RATE_CARD_TIER_ORDER.map((tier) => {
...
export function buildGoliveSteps(state: GoliveStateInput = {}, opts: GoliveStepOptions = {}): GoliveStep[] {
  ...
  const holderRead: Dict = isObj(s.holderRead) ? s.holderRead : {};
