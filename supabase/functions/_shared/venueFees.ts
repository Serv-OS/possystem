// supabase/functions/_shared/venueFees.ts
//
// What a VENUE paid and received on its ServOS Payments card payments, and
// which rows a venue screen shows. PURE: no Deno APIs, no Supabase, no network,
// no device clock.
//
// MIRROR: src/lib/payments/venueFees.js carries the SAME helpers (Deno cannot
// import from src/). KEEP IN SYNC: change both or neither.
...
// for example 0.8% plus 5p in person). That fee is stamped on every payment at
// authorisation as adyen_payments.commission_minor (rate_category names the
// tier), and Adyen's own balance platform bookings matched it on live payments
// (£1.00: 6p, venue received 94p; £2.50: 7p, venue received £2.43).
...
//
// Used by adyen-financial (payments, payouts, payout_detail, statement).

export interface VenueFeeSummary {
  count: number; gross_minor: number; refunds_minor: number; fees_minor: number; receives_minor: number | null;
  fees_rated: number; fees_unrated: number; unrated_gross_minor: number; not_captured: number;
}
export interface VenuePayoutLineFigures { gross_minor: number; fee_minor: number | null; receives_minor: number | null; unrated: boolean }
export interface VenuePayoutSummary { line_count: number; gross_minor: number; fees_minor: number | null; receives_minor: number | null; fees_unrated: number }
// Declared after the interfaces: `node --check` misreads a file whose first
// statement is a bare type alias (resellerRate.ts keeps the same order).
...
const minor = (v: unknown): number => Math.round(numOrNull(v) ?? 0);
...
export function hasSuccessfulCapture(row: Dict | null | undefined): boolean {
  return Array.isArray(row?.applied_mods)
    && (row!.applied_mods as unknown[]).some((m) => String(m).startsWith('CAPTURE:') && String(m).endsWith(':true'));
}
...
export function isUncapturedPayment(row: Dict | null | undefined): boolean {
...
export function isCountedPayment(row: Dict | null | undefined): boolean {
...
export function capturedMinorFor(row: Dict | null | undefined): number | null {
...
function commissionFor(row: Dict | null | undefined): number | null {
  if (!row || !isCountedPayment(row)) return null;
...
function feeAfterRefund(fee: number, amount: number, refunded: number): number {
...
export function venueFeeFor(row: Dict | null | undefined): number | null {
  const commission = commissionFor(row);
  if (commission === null || !row) return null;
...
export function venueReceivesFor(row: Dict | null | undefined): number | null {
  const fee = venueFeeFor(row);
  if (fee === null || !row) return null;
...
export function isVisiblePayment(row: Dict | null | undefined, venueEnv: unknown, includeTest: unknown): boolean {
...
export function summarizeVenueFees(rows: unknown): VenueFeeSummary {
  const out: VenueFeeSummary = {
  ...
  let receives = 0;
  for (const r of (Array.isArray(rows) ? rows : []) as Dict[]) {
    ...
    out.fees_rated++;
    out.fees_minor += fee;
    receives += venueReceivesFor(r) ?? 0;
  }
  out.receives_minor = out.fees_unrated > 0 ? null : receives;
  return out;
}
...
export const PAYOUT_SALE_LINE_TYPES: readonly string[] = Object.freeze(['Settled', 'SettledExternally']);
export const PAYOUT_VENUE_LINE_TYPES: readonly string[] = Object.freeze([
  'Settled', 'SettledExternally',
  'Refunded', 'RefundedExternally', 'RefundedReversed',
  'Chargeback', 'SecondChargeback', 'ChargebackReversed',
]);
const PAYOUT_REFUND_LINE_TYPES: readonly string[] = Object.freeze(['Refunded', 'RefundedExternally', 'RefundedReversed']);
...
export function venuePayoutLine(line: Dict | null | undefined, payment: Dict | null | undefined): VenuePayoutLineFigures | null {
  ...
  if (commission === null || !payment) return { gross_minor: gross, fee_minor: null, receives_minor: null, unrated: true };
...
export function summarizeVenuePayout(figures: unknown): VenuePayoutSummary {
  const out: VenuePayoutSummary = { line_count: 0, gross_minor: 0, fees_minor: null, receives_minor: 0, fees_unrated: 0 };
  for (const f of (Array.isArray(figures) ? figures : []) as (Dict | null)[]) {
    if (!f) continue;
    out.line_count++;
    ...
    else out.receives_minor = (out.receives_minor ?? 0) + (Number(f.receives_minor) || 0);
  }
...
export function venueTimeZoneFor(loc: Dict | null | undefined): string {
...
export function venueCurrentMonth(timeZone: unknown, now: Date = new Date()): string {
...
function venueMidnightMs(y: number, monthIndex: number, day: number, tz: string): number {
...
export function venueMonthBoundsIso(month: unknown, timeZone: unknown): { fromIso: string; toIso: string } | null {
