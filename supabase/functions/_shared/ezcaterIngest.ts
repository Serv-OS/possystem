...
export async function writeEzcaterOrder(sb: any, args: {
  ...
  log?: (...a: unknown[]) => void;
}): Promise<{ ok: true; plan: any; payload: any; isNew: boolean; link: any; attempts: number; stale?: boolean; gone?: string } | { ok: false; error: string }> {
  ...
  for (let attempt = 1; attempt <= WRITE_ATTEMPTS; attempt++) {
    ...
    if (gone.mode === 'skip') return { ok: true, plan, payload: {}, isNew: false, link, attempts: attempt, gone: gone.mode };
    return { ok: true, plan, payload, isNew: !existing && (gone.mode === 'new' || gone.mode === 'unfired'), link, attempts: attempt, ...(gone.mode !== 'new' ? { gone: gone.mode } : {}) };
  }
