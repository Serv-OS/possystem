...
//
// (This was first written for the parked lockdown branch, whose 20260918d file said the same; that
// file is superseded by the fence.) The OLD rule unioned user_profiles.location_id, which was
// writable by the user (and, through a policy that was not row scoped, by any login for any row).
// That made every gift card admin gate spoofable: set a profile venue to someone else's venue and
// you were their staff. The profile venue is now only the venue Back Office opens on, never
// access, here and in the database. File A refuses to run while any login reaches a venue only
// through its profile venue, so nobody real is locked out.
//
