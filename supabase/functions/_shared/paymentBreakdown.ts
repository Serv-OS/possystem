// supabase/functions/_shared/paymentBreakdown.ts
//
// How ONE card payment was split, proven from Adyen's own records. PURE: no
// Deno APIs, no Supabase, no network.
//
// MIRROR: src/lib/payments/paymentBreakdown.js carries the SAME helpers (Deno
// cannot import from src/). KEEP IN SYNC: change both or neither.
// src/lib/payments/paymentBreakdown.test.js is the contract for both copies
// (the Provo £1.00 of 10 Sep 2026 is its main fixture; every input the suite
// builds is replayed through this copy).
//
// Used by adyen-terminal-admin (payment_list, payment_breakdown).
//
// THE MONEY. For each payment Adyen books Balance Platform transfers
...
//   Commission                     the venue fee, to the platform LIABLE account, as
//                                  TWO transfers: the fixed part (value = the rule
//                                  fixedAmount) and the percent part
//   PaymentFee, AcquiringFees, AdyenFees, Interchange, SchemeFee,
//   AdyenCommission, AdyenMarkup   Adyen's fees, from the liable account
// FranPOS keeps its margin (resellerRate.ts); ServOS earns what is left.
// Only captured or booked records count; every record is checked against the
// account it sits on; a read that was not complete is 'incomplete', never a gap.

export interface BreakdownLine { key: string; label: string; minor: number | null; source: 'adyen' | 'rate' | 'franpos' | 'computed'; detail: string | null }
export interface BreakdownReference { label: string; term: string | null; id: string }
...
const curOf = (c: unknown): string => str(c).toUpperCase() || 'GBP';
...
// ── THE WORDS ───────────────────────────────────────────────────────────────
export const BREAKDOWN_LABELS: Readonly<Record<string, string>> = Object.freeze({
...
const VENUE_TYPES: readonly string[] = Object.freeze(['balanceaccount', 'remainder', 'default']);
const FEE_TYPES: readonly string[] = Object.freeze(['paymentfee', 'acquiringfees', 'adyenfees', 'interchange', 'schemefee', 'adyencommission', 'adyenmarkup']);
// Adyen's transfer types that are not the payment itself.
const REFUND_TYPES: readonly string[] = Object.freeze(['refund', 'refundreversal']);
const CHARGEBACK_TYPES: readonly string[] = Object.freeze(['chargeback', 'chargebackreversal', 'secondchargeback', 'chargebackcorrection', 'chargebackreversalcorrection']);
const REVERSAL_TYPES: readonly string[] = Object.freeze(['capturereversal', 'manualcorrection', 'depositcorrection', 'balanceadjustment', 'reserveadjustment']);
// Adyen's transfer statuses. Only BOOKED money counts as money that moved.
const BOOKED_STATUSES: readonly string[] = Object.freeze(['captured', 'booked']);
const PENDING_STATUSES: readonly string[] = Object.freeze(['authorised', 'authorized', 'capturepending', 'bookingpending', 'pending', 'received', 'approvalpending']);
const DEAD_STATUSES: readonly string[] = Object.freeze(['expired', 'capturereversed', 'refused', 'failed', 'cancelled', 'canceled', 'error', 'returned', 'rejected', 'reversed']);

// Money the way a person reads it: "6p", "£1.00", "6c", "$1.00", "€1.00";
// a loss reads "minus 3p", never with a dash.
export function formatMinor(minor: unknown, currency: unknown): string {
  const n = num(minor);
...
// A card as the payment list names it: "Visa Debit ending 1234",
// "Mastercard on Apple Pay". Wallet codes such as mc_applepay read as words.
const BRAND_WORDS: Readonly<Record<string, string>> = Object.freeze({
...
const titleWords = (s: unknown): string => str(s).toLowerCase().replace(/\b[a-z]/g, (c) => c.toUpperCase());
export function paymentCardLabel(card: unknown): string {
  const c: Dict = isObj(card) ? card : {};
...
export function ruleRate(rule: unknown): { percent: number; fixedMinor: number } {
  const r = rule as Dict | null | undefined;
...
export function venueRateLine(rate: { percent?: unknown; fixedMinor?: unknown } | null | undefined, currency: unknown): string {
...
interface FlatTransfer {
  id: string; reference: string; type: string; status: string; direction: string; currency: string;
  minor: number | null; signed: number; platformPaymentType: string; ppt: string; psp: string; balanceAccountId: string;
}
function readTransfer(t: Dict): FlatTransfer {
  const cd: Dict = isObj(t?.categoryData) ? t.categoryData : {};
  const amount: Dict = isObj(t?.amount) ? t.amount : {};
  ...
  return {
    ...
    signed: minor === null ? 0 : (direction === 'outgoing' || (!direction && (value as number) < 0) ? -minor : minor),
...
const REFERENCE_WORDS: Readonly<Record<string, string>> = Object.freeze({
...
// The Adyen word in grey brackets after a reference. Never "commission" on screen.
const termOf = (t: FlatTransfer, fallback: string | null): string | null => (/commission/i.test(t.platformPaymentType) ? null : (t.platformPaymentType || fallback));

// "Tip included.", "Tip and surcharge included."
const includedWords = (tip: number, surcharge: number): string | null => {
...
// ── THE BREAKDOWN ────────────────────────────────────────────────────────────
// See the JS copy for the contract.
export interface BreakdownInput {
  payment?: unknown; transfers?: unknown; rule?: unknown; venueBalanceAccountId?: unknown;
  liableBalanceAccountId?: unknown; reseller?: unknown; readComplete?: unknown;
}
export function buildPaymentBreakdown({ payment, transfers, rule, venueBalanceAccountId, liableBalanceAccountId, reseller, readComplete }: BreakdownInput = {}): Dict {
  const p: Dict = isObj(payment) ? payment : {};
  ...
  const transferRefs: Array<BreakdownReference & { key: string }> = [];
  const refundRefs: Array<BreakdownReference & { key: string }> = [];

  // One record per transfer id (the listings overlap), this payment only.
  const seen = new Map<string, FlatTransfer>();
  ...
  const notInSums = (t: FlatTransfer): boolean => REFUND_TYPES.includes(t.type) || CHARGEBACK_TYPES.includes(t.type) || REVERSAL_TYPES.includes(t.type);
  let otherCurrency = 0;
  let pending = 0;
  let chargebacks = 0;
  let reversals = 0;
  const counted: FlatTransfer[] = [];
  const refunds: FlatTransfer[] = [];
  ...
  // Which account a record sits on. null when it cannot be told.
  const onVenue = (t: FlatTransfer): boolean | null => (venueBa && t.balanceAccountId ? t.balanceAccountId === venueBa : null);
  const onLiable = (t: FlatTransfer): boolean | null => (liableBa && t.balanceAccountId ? t.balanceAccountId === liableBa : null);
  ...
  const commissions: FlatTransfer[] = [];
  ...
  // Records off the pattern: each named, each makes the state mismatch.
  const offPattern: string[] = [];
  ...
  const references = (): BreakdownReference[] => [
  ...
  const notFinished = (state: string, message: string): Dict => ({
    ...base,
    state,
    message,
  ...
  let percentVenue: number | null = null;
  let fixedVenue: number | null = null;
  let partsFrom: string | null = 'adyen';
  const expectedPercent = rate ? halfUp((saleMinor * rate.percent) / 100) : null;
  // Adyen rounds the percent part to the nearest penny; either base is accepted.
  const near = (booked: number, b: number): boolean => (rate ? Math.abs(booked - (b * rate.percent) / 100) <= 0.5 + 1e-9 : false);
  const nearEither = (booked: number): boolean => near(booked, saleMinor) || near(booked, amountMinor);
  const fixedT: FlatTransfer | null = commissions.length === 2 && rate && commissions[0].signed !== commissions[1].signed
    ? commissions.find((t) => t.signed === rate.fixedMinor) || null
    : null;
  const percentT: FlatTransfer | null = fixedT ? (commissions.find((t) => t !== fixedT) ?? null) : null;
  if (fixedT && percentT && nearEither(percentT.signed)) {
    // BOTH records check out against the rate.
    ...
    // A rule with one part priced books ONE transfer, and it can only be that part.
  ...
  let feeMatchesRate: boolean | null = null;
  if (rate) {
  ...
  const r: Dict | null = isObj(reseller) ? reseller : null;
  ...
  // Refunds: money going back out adds, a refund reversal takes off.
  ...
  const split = (venue: number | null, fp: number | null): number | null => (venue === null || fp === null ? null : venue - fp);
  const parts = {
  ...
  // Money on the wrong account that exactly explains the gap is named, not the gap.
...
function padPercent(p: unknown): string {
