...
//
// Who may list, search, issue, spend, put back and fulfil gift cards.
//
// WHY (18 Sep 2026 audit, rounds two and three; database fence stage 1, 19 Sep 2026). Every
// gift function accepted ANY session, including an anonymous one that anybody can mint with the
// public anon key (OnlineCheckout and QrCheckout call signInAnonymously), and took the company
// from a public location_id in the body:
...
//   * gift-issue, gift-import and gift-bulk-create minted cards of any value and handed back the
//     codes; gift-void, gift-config and gift-resend changed any company's cards and settings;
//     gift-reverse-redeem refilled a spent card for ever; gift-fulfill issued a card for an
//     unpaid purchase.
// The parked lockdown branch fixed these, but its device arm read a devices table anybody could
// write. Stage 1 fences that table (20260919a), so "a device" here is exactly the device arm of
// pos_can_access: a session BOUND to a devices row of the venue the call acts for
// (_shared/deviceAuthority.ts). Staff is the database's user_accessible_locations(): a
// user_locations link to the venue, or a verified super admin, never an anonymous session
// (_shared/staffAccess.ts), plus a Platform company role for the venue's company.
//
// THE RULES (enforced always: the legitimate callers are staff, bound devices or code holders):
//   * gift-list: staff only.
//   * gift-lookup by the exact full 16 character code: any session. Typing the code is proof of
...
//   * gift-redeem with a code that resolves: any session (same proof of possession).
//   * gift-redeem by card_id alone (or with a code that does not resolve): staff, a bound device
//     of the venue, or the member whose PROVEN phone (loyalty session token) the card is
//     addressed to. That last arm is the kiosk spending a card it listed after the member's one
//     time code, where the card may have no stored code.
//   * gift-issue, gift-import, gift-bulk-create, gift-config, gift-void, gift-resend: staff only.
//   * gift-reverse-redeem: staff or a bound device of the venue. Never the customer.
//   * gift-fulfill: the service role (the processor webhooks) or staff of the purchase's venue.
//
...
export type GiftRefusal = { ok: false; status: number; error: string; reason: string };

export function decideGiftListAuthority(f: GiftCallerFacts): { ok: true } | GiftRefusal {
...
export function decideGiftLookupAuthority(kind: GiftLookupKind, f: GiftCallerFacts):
  { ok: true; view: 'full' | 'code_holder' } | GiftRefusal {
  if (!f.user) return { ok: false, status: 401, error: 'Unauthorized', reason: 'no_session' };
...
export function decideGiftCardIdAuthority(i: GiftCallerFacts & {
  /** The caller is a bound device of the venue the call acts for (deviceAuthority.ts). */
  device: boolean;
  companyId: string;
...
}): { ok: true; via: 'staff' | 'device' | 'member' } | GiftRefusal {
  ...
  if (i.device) return { ok: true, via: 'device' };
...
// ── Staff only (round three, enforced now) ────────────────────────────────
//
...
export type GiftStaffAuthority = { ok: true } | GiftRefusal;
 ...
 * machine job undo (CheckoutModal, store._reverseTerminalJobGift). So: staff, or a device BOUND to
 * the venue the call names. NOT "the session that made the spend": for an online or kiosk
 * customer that session IS the attacker (spend, eat, reverse). Once per spend is the ledger's job
 * (refund:<original key>, unique per card).
 */
export function decideGiftReverseAuthority(i: GiftCallerFacts & {
  device: boolean;
  /** Why the device arm failed (deviceAuthority.ts), for the reason text. */
  deviceReason?: string | null;
}): { ok: true; via: 'staff' | 'device' } | GiftRefusal {
  if (!i.user) return { ok: false, status: 401, error: 'Unauthorized', reason: 'no_session' };
  if (!i.user.is_anonymous && i.staff) return { ok: true, via: 'staff' };
  if (i.device) return { ok: true, via: 'device' };
  return {
    ok: false, status: 403,
    error: 'Only a till of this venue or a manager can put a gift card spend back. Pair this till again if it should be allowed.',
    reason: i.deviceReason === 'other_venue' ? 'device_other_venue'
      : (i.deviceReason === 'not_bound' ? 'device_not_bound' : (i.user.is_anonymous ? 'anonymous_no_device' : 'no_location_access')),
  };
}
...
export function decideGiftFulfilAuthority(i: { serviceRole: boolean } & GiftCallerFacts):
  { ok: true; via: 'webhook' | 'staff' } | GiftRefusal {
...
// ── Card refunds (stripe-refund, ryft-refund), stage 1 ─────────────────────
//
// Both refunded any payment of any venue for any session: the venue id is public and a customer
// knows their own payment id (it is in the page that took the card). So a customer could pay, eat
// and refund themselves. Their callers are the till's refund (store.refundCheck through
// lib/payments/cardReversal.js: a bound till, MPOS or a Back Office login) and other edge
// functions with the service role. adyen-modify has had this fence since v5.6.79.
export function decideCardRefundAuthority(i: { serviceRole: boolean; device: boolean; deviceReason?: string | null } & GiftCallerFacts):
  { ok: true; via: 'service' | 'staff' | 'device' } | GiftRefusal {
  if (i.serviceRole) return { ok: true, via: 'service' };
  if (!i.user) return { ok: false, status: 401, error: 'Unauthorized', reason: 'no_session' };
  if (!i.user.is_anonymous && i.staff) return { ok: true, via: 'staff' };
  if (i.device) return { ok: true, via: 'device' };
  return {
    ok: false, status: 403,
    error: 'Only a till of this venue or a manager can refund a card payment. Pair this till again, or refund it from Back Office.',
    reason: i.deviceReason === 'other_venue' ? 'device_other_venue'
      : (i.deviceReason === 'not_bound' ? 'device_not_bound' : (i.user.is_anonymous ? 'anonymous_no_device' : 'no_location_access')),
  };
}
