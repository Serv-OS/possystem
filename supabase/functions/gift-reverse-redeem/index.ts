...
//
// Database fence stage 1 (19 Sep 2026, enforced always): any session could call this, and the key
// of a spend (giftcommit:<check>:<card>) is known to the customer who made it. So a card could be
// spent on an order and then refilled, for ever. Now:
//   * only staff of the venue (the database's access rule, _shared/staffAccess.ts) or a device
//     BOUND to the venue (the device arm of pos_can_access, _shared/deviceAuthority.ts) may
//     reverse (_shared/gift-authority.ts decideGiftReverseAuthority). Refusals are logged
//     ([authority] in the function log).
...
import { callerStaffOrDevice, recordAuthority, deviceHintOf } from '../_shared/loyalty-utils.ts';
...
Deno.serve(async (req) => {
  ...
  // ── Who may put a spend back (stage 1, enforced always) ────────────────
  // Before anything is read: staff of the venue, or a device bound to the venue.
  const who = await callerStaffOrDevice(caller, (body.location_id as string) || null, String(companyId));
  const authority = decideGiftReverseAuthority({ user: caller, staff: who.staff, device: who.device, deviceReason: who.deviceReason });
  if (!authority.ok) {
    recordAuthority(authorityLogRow({
      fn: 'gift-reverse-redeem', mode: 'enforce', outcome: 'refused',
      decision: { ok: false, reason: authority.reason, callerKind: caller?.is_anonymous ? (who.deviceReason === 'other_venue' || who.deviceReason === 'not_bound' ? 'device_other' : 'anonymous') : 'user_no_access' },
