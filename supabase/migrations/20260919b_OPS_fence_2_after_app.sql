...
-- statement can run twice; it takes all of its locks up front, busy tables first, with a
-- 3 second wait and a plain "press Run again" message (fix round 3); verification and roll
-- back at the end (the ROLL BACK heading says how to run it).
...
do $locks$
begin
  lock table public.closed_checks, public.order_queue, public.kds_tickets, public.print_jobs,
             public.active_sessions, public.table_reservations, public.bar_tabs
    in access exclusive mode;
exception when lock_not_available or deadlock_detected then
  raise exception 'STOPPED, NOTHING WAS CHANGED. A till was busy with the orders or tables for more than 3 seconds. Wait 10 seconds and press Run again.';
end
$locks$;
