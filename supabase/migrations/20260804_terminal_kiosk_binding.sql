...
-- ── v5.5.884 addendum (APPLIED to prod 23 Jul 2026) ──────────────────────────
-- set_terminal_settings must ALSO accept a kiosk binding: the terminal Settings
-- panel is a whole-settings write that re-submits the current bound device, so a
-- terminal assigned to a kiosk had EVERY settings save rejected with "that till
-- is not a POS device at this terminal's venue". Same one-line relaxation as
-- set_terminal_bound_device; body otherwise byte-identical to 20260723 § 3.
