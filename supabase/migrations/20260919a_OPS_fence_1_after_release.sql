...
--      device switched on in the last 2 hours has not reported the app release (fix round 3:
--      the test is the capability the app itself records, client_caps 'fence_v1', or a device
--      secret it already holds, NOT a version string, because 5.9.10 and 5.9.11 both shipped
--      without a line of the fence app and every till would have sailed through); takes its
--      locks up front, busy tables first; 3 second lock wait with a plain "press Run again"
--      message. It also remembers when it first ran: file 2 refuses to run until a full day
--      later. It needs 20260919_OPS_fence_0_caps.sql (runbook step 1b, which goes out with the
--      release so the app can record its capability) and says so if it is missing.
...
begin
  ...
  -- exactly what our own page asked for. It is written up in docs/FENCE_STAGE_1.md.
...
declare
  ...
  -- in docs/FENCE_STAGE_1.md, with the Back Office query that names the unconfigured rows.
