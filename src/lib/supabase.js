...
// 18 Sep 2026 (round three): a KIOSK does not re-claim on boot. It claims once, at pairing, and
// the code is then cleared on its devices row (KioskSurface). Round two kept the code so a kiosk
// could re-claim, but the live claim_device (20260713c) matches ANY row holding a code and
// overwrites device_uid, so a second tablet typing the same code would take the link on every
// boot. A kiosk whose link is missing is re-paired once from Back Office.

/**
 * The id this browser was paired as (its rpos-device id, or its rpos-kiosk-id), or null. Sent as
 * `device_hint` with loyalty and gift card calls so caller_authority_log can name the till or
 * kiosk that needs re-pairing. The server never trusts it for anything else.
 */
export const localDeviceHint = () => {
  try {
    const dev = JSON.parse(localStorage.getItem('rpos-device') || 'null');
    if (dev && dev.id && dev.id !== 'admin' && !dev.adminMode) return String(dev.id);
  } catch { /* fall through */ }
  try { return localStorage.getItem('rpos-kiosk-id') || null; } catch { return null; }
};
