// src/lib/ezcaterMatch.js
//
// PURE matching rules for ezCater order lines against our own menu. No imports,
// no supabase client, no fetch, no clock reads. Everything is a function of its
// arguments, so the browser app and the Deno edge function can both run it and
// always agree about what matched what.
//
// MIRROR: supabase/functions/_shared/ezcaterMatch.ts holds the same rules for
// the edge function (the function is deployed on its own, sometimes by pasting
// the files under supabase/functions, so it cannot import out of src/). The two
// files are held together by src/lib/ezcaterMatchParity.test.js, which runs one
// shared case table through both and compares every output. Change one, change
 ...
 */
export const CONTAINER_WORDS = Object.freeze(['tray', 'pan', 'size']);

/** Every word that is dropped when it trails a name. */
export const TRAILING_DROP = Object.freeze(
 ...
 */
export function sizeWordOf(value) {
 ...
 */
export const GENERIC_SIZE_NAMES = Object.freeze([
 ...
 */
export function ezLineName(name, sizeName) {
 ...
 */
export function ourSizeName(parentName, sizeName) {
...
const samePrice = (a, b) => a !== null && b !== null && Math.abs(a - b) < 0.005;

/** Name only, one candidate name against one of ours. Returns the raw parts. */
function nameScore(theirName, ourName) {
 ...
 */
export function scoreMatch(theirName, ourItem) {
 ...
 */
export function suggestMatches(theirLine, ourItems, opts) {
  const o = opts || {};
  const limit = Number.isFinite(o.limit) ? Math.max(0, Math.floor(o.limit)) : DEFAULT_LIMIT;
 ...
 */
export function indexItemCodes(ourItems) {
 ...
 */
export function findItemCodeMatch(codes, value) {
...
function exactNameMatches(theirName, ourItems) {
 ...
 */
export function autoLinkDecision(theirLine, ourItems, existingLinks, opts) {
  const o = opts || {};
  ...
  if (posItemId && known.has(posItemId)) {
    const out = { action: 'linked', itemId: posItemId, reason: 'their menu has our id', source: 'posItemId' };
  ...
  if (exact.length > 1) {
    const out = { action: 'suggest', reason: 'more than one item with that name' };
    if (stale) out.stale = true;
    return out;
  }
 ...
 */
function flattenOptions(ourGroups) {
  const out = [];
...
function byOptionSuggestion(a, b) {
  if (b.score !== a.score) return b.score - a.score;
  if (a._key !== b._key) return a._key < b._key ? -1 : 1;
 ...
 */
export function matchOptions(theirMod, ourGroups, opts) {
  const o = opts || {};
  const limit = Number.isFinite(o.limit) ? Math.max(0, Math.floor(o.limit)) : DEFAULT_LIMIT;
  const minScore = Number.isFinite(o.minScore) ? o.minScore : DEFAULT_MIN_SCORE;
 ...
 */
function autoLinkOption(theirMod, ourGroups, existingLinks, minScore, itemCodes) {
  const their = theirParts(theirMod);
  ...
  if (exact.length > 1) {
    const out = { action: 'suggest', reason: 'more than one option with that name' };
    if (stale) out.stale = true;
    return out;
  }
 ...
 */
export function applyLinks(lines, links, itemCodes) {
  ...
  return list.map((line) => {
    ...
    const mods = (Array.isArray(src.mods) ? src.mods : []).map((mod) => {
      ...
      let mSource = mItemId ? 'posItemId' : null;
 ...
 */
export function countMatches(lines) {
  const list = Array.isArray(lines) ? lines : [];
  let matched = 0;
  let modTotal = 0;
  let modMatched = 0;
