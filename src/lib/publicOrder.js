...
export async function settleQrTabWithFallback({
  ...
  for (let attempt = 0; attempt < Math.max(1, retries); attempt++) {
    ...
    // C23 (fix round 2): 'short' carries what the capture paid and what the tab is worth.
    if (last.reason === 'short') { last.paidMinor = Number(data?.paid_minor) || 0; last.dueMinor = Number(data?.due_minor) || 0; }
...
// The server's _fence_bool: 'true', 't', '1', 'yes', 'y', 'on' (any case), or a real true.
...
export function publicOrderRefusalMessage(placed, fallback) {
  ...
  // C22 (fix round 2): the round would take the tab past its card hold. Nothing was charged for it.
  if (r === 'over_hold') return m || 'This round would take your tab past its card hold. Close your tab and start a new one, or ask a member of staff.';
...
/**
 * C22 (fix round 2): the card hold for a NEW QR tab, in whole currency units. place_public_order
 * refuses any round that would take the tab past its hold ('over_hold'), the first round too, so
 * the hold is at least the first round: max(the venue's hold, the minimum, the first round's total
 * rounded up). A hold of the venue's size still covers later rounds as before.
 */
export function tabHoldFor({ configured = 0, minimum = 25, firstRound = 0 } = {}) {
  const num = (v) => { const n = Number(v); return Number.isFinite(n) && n > 0 ? n : 0; };
  return Math.max(num(configured), num(minimum), Math.ceil(num(firstRound) - 1e-9));
}

/** C23 (fix round 2): what the phone says after the card was captured but the tab did not close. */
export function tabCloseRefusalMessage(settled, { tabRef = '' } = {}) {
  const r = settled && settled.reason;
  const ref = tabRef ? ` (${tabRef})` : '';
  if (r === 'short') {
    return (settled && settled.message) || 'Your card paid part of this tab. A member of staff will settle the rest with you.';
  }
  if (r === 'not_yours') {
    return `We charged your card. Only the phone that opened this tab, or one that joined it with the table code, can close it here, so the venue will close it on the till. Please show this screen to staff${ref}.`;
  }
  return `We charged your card. The venue will close your tab on the till. Please show this screen to staff${ref}.`;
}
...
export function trackerPaymentChecking(row) {
  ...
  // C21 (fix round 2): 'short' reads as checking for the customer (order_track_row already
