...
test('the list never carries fulfilled_code, and gift-list checks staff before reading', () => {
  ...
  const i = fn.indexOf("if (body.action === 'purchases') {");
  assert.ok(i > 0);
  const block = fn.slice(i, i + 900);
  assert.ok(block.indexOf('isCompanyStaff(caller, companyId)') < block.indexOf(".from('gift_card_purchases')"), 'the staff check comes first');
