...
export function readPhone(raw) {
  ...
  const no = (reason) => ({ phone: null, local: null, e164: null, ok: false, empty: false, assumed: false, reason });
  ...
  const dialled = (national, assumed) => {
 ...
 * Takes customer rows ({ phone, phone_raw, email }), plain strings, a
 * { phones, emails } pair, or a Set this function already built, and gives back
 * a Set of 'p:...' / 'e:jane@x.com'. Running it on its own output changes
 * nothing.
 *
 * ONE PERSON GETS SEVERAL PHONE KEYS, on purpose. See phoneKeys: their number
 * can be on file as what the app's rule produced, as the E.164 form an older
 * import wrote, or as whatever they typed in phone_raw, and missing any of
 * those makes a second customer out of somebody we already have.
 */
export function buildExistingKeys(source) {
  ...
  const addPhone = (v) => { const list = phoneKeys(v); for (let i = 0; i < list.length; i++) keys.add('p:' + list[i]); };
  ...
  if (Array.isArray(source)) {
    for (let i = 0; i < source.length; i++) {
      ...
      if (typeof item === 'string') {
        ...
        addPhone(item.phone);
        addPhone(item.phone_raw);
        addPhone(item.phoneRaw);
        addEmail(item.email);
      }
    }
    return keys;
 ...
 */
export function matchesExisting(row, keys) {
  if (!row || !keys || typeof keys.has !== 'function') return false;
  const forms = [row.phone, row.phoneE164, row.phoneLocal];
  for (let i = 0; i < forms.length; i++) if (forms[i] && keys.has('p:' + forms[i])) return true;
  return !!(row.email && keys.has('e:' + row.email));
}
