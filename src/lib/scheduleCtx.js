 ...
 *
 * `now` (optional, a Date) evaluates the context at that instant instead of the
 * current clock. Only tests and replays should pass it. Every production caller
 * omits it, so the venue-clock invariant is untouched.
 */
export function buildScheduleCtx(timezone = 'Europe/London', now) {
  ...
  try {
    ...
    const isoDay = ((at.getDay() + 6) % 7) + 1; // JS 0=Sun → ISO 7=Sun
    return {
      nowMinutes: at.getHours() * 60 + at.getMinutes(),
      isoDay,
      ymd: at.toISOString().slice(0, 10),
    };
  }
}
