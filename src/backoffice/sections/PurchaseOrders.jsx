    ...
    <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr', height: '100%', minHeight: 0, background: 'var(--bg0)' }}>
      <aside style={{ borderRight: '1px solid var(--bdr)', background: 'var(--bg1)', display: 'flex', flexDirection: 'column', minHeight: 0 }}>
        <div style={{ padding: 14 }}>
          <button onClick={newPO} style={{ width: '100%', padding: '9px 0', borderRadius: 7, background: 'var(--acc)', color: '#fff', border: 0, fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>+ New purchase order</button>
        </div>
          ...
          {pos.map(p => (
            <div key={p.id} onClick={() => { setDraft(structuredClone(p)); setSelId(p.id); }} style={{ padding: '11px 16px', cursor: 'pointer', borderLeft: '3px solid ' + (p.id === selId ? 'var(--acc)' : 'transparent'), background: p.id === selId ? 'var(--bg2)' : 'transparent' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <span style={{ fontSize: 10, fontWeight: 700, color: STATUS_COLOR[p.status] }}>{p.status}</span>
              </div>
        ...
        {draft && (
          <div style={{ padding: '22px 26px', maxWidth: 900 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 16, flexWrap: 'wrap' }}>
              <h1 style={{ fontSize: 21, fontWeight: 600, margin: 0, color: 'var(--t1)', flex: 1 }}>{draft.id ? `Order · ${supName(draft.supplierId)}` : 'New purchase order'}</h1>
              {draft.id && <span style={{ fontSize: 12, fontWeight: 700, color: STATUS_COLOR[draft.status] }}>{draft.status}</span>}
            </div>
            ...
            {draft.status === 'RECEIVED' && <div style={{ marginTop: 12, fontSize: 12, color: 'var(--grn, #16a34a)' }}>✓ Received into stock — costs updated. See each item's Stock tab for the delivery movement.</div>}
          </div>
        )}
      </main>
    </div>
