...
// Lightspeed parity: a payout list (date, gross, fees, what you receive, batch)
// that clicks through to every transaction in the payout.
//
// VENUE FEES (11 Sep 2026). The settlement report's own fee and net columns
// are Adyen's cost to the platform and never reach this tab. The server sends
// only the venue's figures: the card rate fee of each payment a line matches
// by PSP reference, and what the venue receives. The Fee column only shows
// when at least one line or payout has a venue fee. Cost lines (Adyen charges,
// account movements) are left out server side.
// TIMES: payment times are on the venue clock (the answer's `timezone`);
// payout dates are calendar days and never shift with a zone.
//
...
import { formatVenueWhen, formatVenueCsvWhen, formatCalendarDate, venueZoneLabel } from '../../../lib/payments/venueTime';

// Payout status words. 'settled' comes from settlement batches; 'initiated',
// 'sent' and 'failed' from bank transfers (adyen-bp-webhook payoutStatus).
const PAYOUT_STATUS = { settled: 'Settled', initiated: 'On its way', sent: 'Sent to your bank', failed: 'Failed' };
const payoutStatusLabel = (s) => PAYOUT_STATUS[s] || 'Unknown';
...
// Only payment lines reach a venue; cost and account lines are left out.
...
const csvMoney = (v) => (hasValue(v) ? ((Number(v) || 0) / 100).toFixed(2) : '');

// Shared setup steps: the one-time setup that makes settlement reports (and
// therefore payouts) start flowing.
export function SettlementSetupSteps() {
  const steps = [
    'Log in to your payment provider’s Customer Area with an admin account. Test accounts use ca-test.adyen.com. Live accounts use ca-live.adyen.com.',
  ...
  return (
    <div style={{ background: 'var(--bg1)', border: '1px solid var(--bdr)', borderRadius: 12, padding: 18, marginTop: 14 }}>
      ...
      <p style={{ fontSize: 12, color: 'var(--t3)', margin: '10px 0 0', lineHeight: 1.5 }}>
        ...
        fill themselves, usually the next working day.
      </p>
    </div>
  );
}
...
const unratedNote = { fontSize: 12, color: 'var(--t3)', margin: '0 0 14px' };
...
export default function AdyenPayouts() {
  ...
  const [detail, setDetail] = useState(null);      // { payout, lines, timezone }
  ...
  const m = (minor, cur) => (hasValue(minor) ? money((Number(minor) || 0) / 100, cur || 'GBP') : '—');
  ...
  // ── Drill-down: one payout, every payment line inside it ─────────────────
  if (detail) {
    const p = detail.payout || {};
    const lines = detail.lines || [];
    const cur = p.currency || 'GBP';
    const showFee = lines.some((l) => hasValue(l.fee_minor));
    const grid = showFee ? '1.1fr 1.2fr 1.1fr 0.8fr 0.7fr 0.9fr' : '1.1fr 1.2fr 1.1fr 0.9fr 0.9fr';
    const exportLines = () => {
      const headers = [
        { label: 'Type', key: (l) => lineTypeLabel(l.line_type) },
        { label: `Payment taken (${tz})`, key: (l) => formatVenueCsvWhen(l.payment?.taken_at ?? l.payment?.created_at, tz) },
        { label: 'Card', key: (l) => cardLabel(l.payment?.card) },
        { label: 'Gross', key: (l) => csvMoney(l.gross_minor) },
        ...(showFee ? [{ label: 'Fee', key: (l) => csvMoney(l.fee_minor) }] : []),
        { label: 'You receive', key: (l) => csvMoney(l.receives_minor) },
    ...
    return (
      <div style={{ padding: '4px 0' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, flexWrap: 'wrap', marginBottom: 12 }}>
          <div style={{ fontSize: 16, fontWeight: 800, color: 'var(--t1)' }}>Payout, {formatCalendarDate(p.payout_date)}</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(150px,1fr))', gap: 12, marginBottom: (Number(p.fees_unrated) || 0) > 0 ? 8 : 18 }}>
          <StatTile label="Gross" value={m(p.gross_minor, cur)} />
          {hasValue(p.fees_minor) && (
            <div title="What you paid on your card rates">
              <StatTile label="Fees" value={m(p.fees_minor, cur)} color="var(--amb,#e8a020)" />
            </div>
          )}
          <StatTile label="You receive" value={m(p.receives_minor, cur)} color="var(--grn)" />
        </div>
        {(Number(p.fees_unrated) || 0) > 0 && <p style={unratedNote}>Some payments have no fee on record yet.</p>}
        ...
        {lines.length === 0 ? (
          <EmptyState icon="🏦" message="No payment lines recorded for this payout." />
        ) : (
          <div style={{ border: '1px solid var(--bdr)', borderRadius: 10, overflowX: 'auto' }}>
            <div style={{ minWidth: 640 }}>
              <div style={{ display: 'grid', gridTemplateColumns: grid, padding: '9px 14px', background: 'var(--bg2)', ...th }}>
                <div>Type</div><div>Payment taken</div><div>Card</div>
                <div style={{ textAlign: 'right' }}>Gross</div>
                {showFee && <div style={{ textAlign: 'right' }}>Fee</div>}
                <div style={{ textAlign: 'right' }}>You receive</div>
              </div>
              {lines.map((l) => (
                <div key={l.id} style={{ display: 'grid', gridTemplateColumns: grid, fontSize: 13, padding: '9px 14px', borderTop: '1px solid var(--bdr)', alignItems: 'center' }}>
                  <div style={{ color: 'var(--t2)', fontWeight: 600 }} title={l.line_type}>{lineTypeLabel(l.line_type)}</div>
                  <div style={{ color: 'var(--t3)', fontSize: 12 }}>{formatVenueWhen(l.payment?.taken_at ?? l.payment?.created_at, tz)}</div>
                  ...
                  {showFee && <div style={{ ...numCell, color: hasValue(l.fee_minor) ? 'var(--amb,#e8a020)' : 'var(--t4)' }}>{m(l.fee_minor, l.currency)}</div>}
                  <div style={{ ...numCell, color: (Number(l.receives_minor) || 0) < 0 ? 'var(--red)' : (hasValue(l.receives_minor) ? 'var(--t1)' : 'var(--t4)') }}>{m(l.receives_minor, l.currency)}</div>
                </div>
              ))}
            </div>
          </div>
        )}
        <div style={{ fontSize: 11, color: 'var(--t4)', marginTop: 10 }}>{`Times are ${venueZoneLabel(tz)} time`}</div>
      </div>
    );
  }
  ...
  const showFees = rows.some((p) => hasValue(p.fees_minor));
  const anyUnrated = rows.some((p) => (Number(p.fees_unrated) || 0) > 0);
  const grid = showFees ? '1.2fr 0.7fr 0.9fr 0.9fr 0.9fr 1fr 0.5fr' : '1.2fr 0.7fr 0.9fr 0.9fr 1fr 0.5fr';

  const exportCsv = () => {
    const headers = [
      { label: 'Date', key: (p) => p.payout_date || '' },
      { label: 'Batch', key: (p) => (p.batch_number != null ? String(p.batch_number) : '') },
      { label: 'Gross', key: (p) => csvMoney(p.gross_minor) },
      ...(showFees ? [{ label: 'Fees', key: (p) => csvMoney(p.fees_minor) }] : []),
      { label: 'You receive', key: (p) => csvMoney(p.receives_minor) },
      { label: 'Currency', key: (p) => p.currency || 'GBP' },
      { label: 'Status', key: (p) => payoutStatusLabel(p.status) },
  ...
  return (
    <div style={{ padding: '4px 0' }}>
      ...
      {anyUnrated && <p style={unratedNote}>Some payments have no fee on record yet.</p>}
      <div style={{ border: '1px solid var(--bdr)', borderRadius: 10, overflowX: 'auto' }}>
        <div style={{ minWidth: 640 }}>
          <div style={{ display: 'grid', gridTemplateColumns: grid, padding: '9px 14px', background: 'var(--bg2)', ...th }}>
            <div>Date</div><div>Batch</div>
            <div style={{ textAlign: 'right' }}>Gross</div>
            {showFees && <div style={{ textAlign: 'right' }}>Fees</div>}
            <div style={{ textAlign: 'right' }}>You receive</div>
            <div style={{ paddingLeft: 14 }}>Status</div><div />
          </div>
          {rows.map((p) => (
            <button key={p.id} onClick={() => openDetail(p)} disabled={detailLoading} style={{
              display: 'grid', gridTemplateColumns: grid, width: '100%',
              ...
              <div style={{ color: 'var(--t1)', fontWeight: 600 }}>{formatCalendarDate(p.payout_date)}</div>
              <div style={{ color: 'var(--t3)' }}>{p.batch_number != null ? p.batch_number : '—'}</div>
              <div style={numCell}>{m(p.gross_minor, p.currency)}</div>
              {showFees && <div style={{ ...numCell, color: hasValue(p.fees_minor) ? 'var(--amb,#e8a020)' : 'var(--t4)' }}>{m(p.fees_minor, p.currency)}</div>}
              <div style={{ ...numCell, color: hasValue(p.receives_minor) ? 'var(--grn)' : 'var(--t4)' }}>{m(p.receives_minor, p.currency)}</div>
