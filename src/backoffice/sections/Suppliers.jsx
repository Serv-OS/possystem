...
export default function Suppliers() {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr', height: '100%', minHeight: 0, background: 'var(--bg0)' }}>
      <aside style={{ borderRight: '1px solid var(--bdr)', background: 'var(--bg1)', display: 'flex', flexDirection: 'column', minHeight: 0 }}>
        <div style={{ padding: 14, position: 'relative' }}>
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search suppliers…" style={{ ...field, paddingRight: 40 }} />
          <button onClick={() => { setDraft(blank()); setSelId(null); }} style={{ position: 'absolute', top: 14, right: 16, width: 28, height: 28, borderRadius: 6, background: 'var(--acc)', color: '#fff', border: 0, fontSize: 18, cursor: 'pointer' }}>+</button>
        </div>
      ...
      <main style={{ overflowY: 'auto', minHeight: 0 }}>
        ...
        {draft && (
          <div style={{ padding: '22px 26px', maxWidth: 640 }}>
            <h1 style={{ fontSize: 21, fontWeight: 600, margin: '0 0 18px', color: 'var(--t1)' }}>{draft.name || 'New supplier'}</h1>
            ...
            <div style={{ marginTop: 14 }}>
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                {DAYS.map(([d, l]) => {
                  ...
                  return <button key={d} onClick={() => toggleDay(d)} style={{ flex: 1, padding: '7px 0', fontSize: 12, borderRadius: 6, cursor: 'pointer', background: on ? 'var(--acc-d, rgba(249,115,22,0.12))' : 'var(--bg2)', color: on ? 'var(--acc)' : 'var(--t3)', border: '1px solid ' + (on ? 'var(--acc)' : 'var(--bdr)') }}>{l}</button>;
                })}
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 18 }}>
              <button onClick={save} disabled={saving} style={{ padding: '10px 22px', borderRadius: 8, background: 'var(--acc)', color: '#fff', border: 0, fontSize: 14, fontWeight: 700, cursor: 'pointer', opacity: saving ? 0.6 : 1 }}>{saving ? 'Saving…' : draft.id ? 'Save changes' : 'Create supplier'}</button>
              {draft.id && <button onClick={archive} style={{ padding: '10px 16px', borderRadius: 8, background: 'var(--bg2)', border: '1px solid var(--bdr)', color: 'var(--t2)', fontSize: 13, cursor: 'pointer' }}>Archive</button>}
            </div>
          </div>
        )}
      </main>
