    ...
    try {
      ...
      const legacySettle = async () => {
        ...
        try {
          ...
          await writeClosedCheckRow(supabase, {
      ...
      if (!settled.ok) {
        ...
        // Fence C23 (fix round 2): 'short' (the capture paid part of the tab, staff settle the
        // rest; the rounds are marked for them) and 'not_yours' (only the opener or a member may
        // close it here) in plain words. Never "pay again".
        setError(tabCloseRefusalMessage(settled, { tabRef: tab.tab_ref || '' }));
        return;
      }
