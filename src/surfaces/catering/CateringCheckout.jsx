...
export default function CateringCheckout({ location, cfg, cart, taxRates, theme, cur, fulfilment, eventDate, eventTime, subtotal, onBack }) {
  ...
  const buildCustomer = (pay) => ({
    ...
    ...(promo.trim() ? { promo_code: promo.trim().toUpperCase() } : {}), ...(tip ? { tip } : {}), ...(deliveryFee ? { delivery_fee: deliveryFee } : {}),
    ...pay,
  });
  ...
  const finalizeNow = async (paymentIntent) => {
    ...
    try {
      ...
      const closedCheck = {
        ...
        order_type: fulfilment, customer: buildCustomer(pay), items: buildItems().map((i) => ({ ...i, voided: false })), discounts: [],
