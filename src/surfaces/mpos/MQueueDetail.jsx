...
export default function MQueueDetail({ order, onBack }) {
  ...
  const advance = () => {
    ...
    if (next === 'collected' && checking) {
      showToast?.(`${PAYMENT_CHECKING_LABEL}: check it in the Orders Hub on a till before handing it over. Do not charge it again.`, 'error');
      return;
    }
  ...
  return (
    <div style={Sx.shell}>
        ...
        <div style={{ padding:'12px 14px 24px' }}>
          ...
          {checking && (
            <div style={{ marginTop:6, fontSize:11, fontWeight:700, color:'#b45309', textAlign:'right' }}>
              {PAYMENT_CHECKING_LABEL} · do not charge again
            </div>
          )}
      ...
      <div style={Sx.bottom}>
        {advanceLabel ? (
          <button onClick={advance} style={Sx.btnPrim}>{(nextStatus(status) === 'collected' && checking) ? PAYMENT_CHECKING_LABEL : (nextStatus(status) === 'collected' && !paid) ? '💳 Take payment to collect' : advanceLabel}</button>
        ) : (
