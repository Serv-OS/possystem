...
export default function StaffSurface() {
  ...
  return (
    ...
    >
      {tab === 'shifts' && <ShiftsTab snap={snap} />}
      ...
      <nav style={{ position: 'fixed', left: 0, right: 0, bottom: 12, display: 'flex', justifyContent: 'center', pointerEvents: 'none' }}>
        <div className="sv-glass" style={{ display: 'flex', gap: 4, padding: 6, borderRadius: 999, pointerEvents: 'auto' }}>
          {[['shifts', 'clock', 'Shifts'], ['news', 'status', 'News'], ['sheets', 'clipboard', 'Timesheets'], ['me', 'team', 'Me']].map(([k, icon, lbl]) => (
...
function ShiftsTab({ snap }) {
  const shifts = snap?.shifts || [];
  if (!shifts.length) return <div className="sv-glass" style={glass}>No published shifts in the next two weeks. When the rota is published, they show here.</div>;
  return (
