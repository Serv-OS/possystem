...
const CHANGELOG = [
  ...
  },
  {
    ...
    changes: [
      ...
      'New toggle in Channels → Delivery channels (HubRise) → Order handling: “Always print a customer receipt for delivery orders” (on by default).',
    ],
  },
  {
  ...
  },
  {
    ...
    changes: [
      ...
      'Next: the back-office “Menu appearance” screen to choose the brand colour, header style, header photo and logo shape with a live preview.',
    ],
  },
  {
  ...
  },
  {
    ...
    changes: [
      ...
      'CANCEL ON TIMEOUT: When the 5-minute card payment timeout fires, the kiosk now calls stripe-cancel-reader-action to clear the pending payment from the physical reader. Also cancels on Back button press and component unmount.',
      'SKIP DOUBLE TIP: Kiosk sends skip_tipping:true — reader skips tip prompt since kiosk already collected it.',
      'CARD READER: Server-side stripe-process-payment-on-reader flow with kioskId as pos_device_id.',
    ],
  },
  {
  ...
  },
  {
    ...
    changes: [
      ...
      'Name, Phone, and Email now show red asterisks (*) to indicate they\'re required. Error message specifies which fields are missing.',
    ],
  },
  {
  ...
  },
  {
    ...
    changes: [
      ...
      'TAB-OPEN PRE-AUTH NOW SAVES THE CARD — QrCheckout passes setup_future_usage="off_session" when creating the manual-capture PI. Stripe attaches the payment_method to the connected account on confirm; the id is stashed in order_queue.customer.payment_method_id. Without this we couldn\'t do off_session re-charges. Existing tabs opened before v5.5.160 don\'t have this so they fall back to the old "shortfall must be collected separately" path with a clear error.',
      'MINIMUM PRE-AUTH OF £25 — was failing silently when location.qr_tab_pre_auth_amount was 0/null (Stripe rejects amount=0 PIs). Customer would tap "open tab" → spinner → generic "could not start payment". Floor of £25 ensures we always have SOMETHING held; if the bill exceeds it the overage path picks up the rest. Override per-venue via the BO setting.',
      'NEW ENDPOINT /api/stripe-charge-overage — generic "charge X more on a saved payment_method on this connected account" wrapper. Returns clear error JSON for declines (insufficient_funds, lost_card, do_not_honor) so the operator can see WHY the auto-charge failed and decide whether to collect via terminal/cash/manual link. Uses the platform STRIPE_SECRET_KEY + Stripe-Account header (same shape as stripe-capture).',
      'STRIPE-CAPTURE NOW CLAMPS — fetches the PI first, reads amount_capturable, and never sends a capture > available. Returns shortfall + capped_from_overage flags in the success response so the caller knows what still needs collecting. Means even an old tab with no saved card no longer ERRORS at close — it captures what it can and the operator gets a clear "£X outstanding" toast.',
      'EDGE FN stripe-create-payment-intent NOW ACCEPTS setup_future_usage — passes through to Stripe. Backwards-compatible (default omits the field, normal one-shot charges unchanged). Deploy this Supabase fn after the Vercel push or QR tab-open will fail with "unknown parameter".',
    ],
  },
  {
    version: '5.5.159', date: '9 May 2026', label: 'QR end-to-end polish — dev.pos-up.com base URL · QR section catches pay-now too · settle-from-start · loud kitchen_routed_at warning · Stripe env-var diagnostics',
    changes: [
      'QR CODES NOW POINT AT dev.pos-up.com — buildQrUrl in BO Online Ordering generates `https://dev.pos-up.com/?surface=qr&loc=<slug>&t=<id>` (was `https://<slug>.pos-up.com/?surface=qr&t=<id>` which assumed per-tenant subdomain DNS that we don\'t have yet). Same change applied to the BO `Preview online` / `Preview QR` links and the in-Office overview links. Once *.pos-up.com routing is live we can flip back; for now everything resolves on the shared dev host so you can scan a printed QR at an actual table and it works.',
      'EVERY GENERATED QR NOW SHOWS ITS URL IN THE BO — the table-QR grid in Online Ordering used to be a row of buttons that just downloaded the JPEG, with no way to verify what URL was actually encoded. Now each card surfaces the full URL in mono-spaced text + table id + a Copy URL button. Catches the case-mismatch class of bug (`?t=t4` vs DB `T4`) instantly: you SEE that the URL is `t=T4` and that matches the DB.',
      'ORDERS HUB QR SECTION NOW CATCHES PAY-NOW + OPEN-TAB — was filtering by `tab_open === true` so QR pay-now orders fell through into Walk-in/Takeaway/Delivery (operator confusion: "where did the QR order go?"). New rule: `source==="qr"` puts it in the QR section regardless. Open-tab rounds still pool by payment_intent_id (one card per tab); pay-now orders each get their own card under the same QR header. PAID badge + Advance button on pay-now cards, HELD badge + Close & charge button on open-tab cards. Walk-in section now excludes ALL QR.',
      'SETTLE-FROM-START — when a customer scans the QR and arrives at a table that already has an open tab (regardless of which phone opened it), the Confirm-Table screen now surfaces the open bill(s) with a one-tap Settle button. Lets a different person at the table close the tab even when the original phone isn\'t around. Existing localStorage-stash silent-resume still triggers when the SAME phone scans (instant Welcome-back); the start-screen settle path is the fallback for everyone else. One DB query in parallel with the resume check; doesn\'t block anything.',
      'LOUD WARNING WHEN order_queue.kitchen_routed_at COLUMN IS MISSING — silent console.warn was being missed for weeks while every venue with N master devices ate N duplicate kitchen tickets per round. routeKioskOrderPrints now also fires a 15-second on-screen ERROR toast (only once per session via `_colWarningShown` flag) with the EXACT one-line SQL to fix it: `alter table order_queue add column if not exists kitchen_routed_at timestamptz;`. This is the THIRD time this bug has bitten — the fix is one line, but it\'s easy to forget when no one tells you.',
      'STRIPE-CAPTURE NOW TRIES MULTIPLE ENV VAR NAMES + REPORTS WHAT\'S ACTUALLY SET — operator kept getting `Stripe not configured` despite insisting STRIPE_SECRET_KEY was in Vercel. The 500 response now: (a) tries STRIPE_SECRET_KEY → STRIPE_API_KEY → STRIPE_TEST_KEY → STRIPE_LIVE_KEY in fallback order, so a venue using a slightly-different name still works, (b) when none match, lists every env var on the running Vercel function whose name contains "stripe" (saves the "I added it but it\'s not picking up" guessing game — you SEE what the function sees), (c) reminds you that Vercel env vars and Supabase Edge Function secrets are SEPARATE stores and that env-var changes don\'t apply until the next deploy.',
    ],
  },
  {
  ...
  },
  {
    ...
    changes: [
      ...
      'FORCE-CLOSE NOW SUMS ALL ROUNDS — was capturing only the single row\'s total, missing any extra rounds the customer added via "Add more". forceCloseQrTab now sums totals across every order_queue row sharing that payment_intent_id, applies any auto-surcharge if past the threshold, captures up to the held pre-auth (Stripe rejects over-capture — clear shortfall warning if the bill exceeded the hold), marks every round collected, writes ONE consolidated closed_check.',
      'FLOOR-PLAN TABLE SESSION SYNC — QR tabs now also surface on TablesSurface (the floor-plan view) as in-service tables, not just on the OrdersHub QR-tabs section. New helper `syncQrTableSession(locationId, tableId)` recomputes the active_sessions row for the table from every open QR round and upserts it; called after first tab open, every Add More round, and on force-close (the close path clears the session if no other QR rounds remain). Multiple sub-tabs at the same table merge into one combined floor-plan view ("Table 4 has 5 items going").',
      '⚠ STRIPE_SECRET_KEY ON VERCEL — if force-close errors with "Stripe not configured" even though the env var exists, it usually means the var was added AFTER the last deploy. Vercel serverless functions only see env vars that existed at build time. Fix: redeploy (Vercel dashboard → Deployments → latest → "Redeploy"), or push any commit (this commit will do that). Also confirm the var is scoped to BOTH Production AND Preview environments — default is Production-only and a preview URL won\'t see it.',
      '⚠ DUPLICATE PRINTS — if you\'re still seeing 3× kitchen tickets per round, the order_queue table is missing the `kitchen_routed_at` column (atomic dedup falls back to in-memory per-device, so N master devices = N prints). One-line SQL: `alter table public.order_queue add column if not exists kitchen_routed_at timestamptz;` Run on ops Supabase, idempotent.',
    ],
  },
  {
  ...
  },
  {
    ...
    changes: [
      ...
      'TAB RESUME SCREEN — "👋 Welcome back to your tab — total so far £X.XX" with a clean breakdown of every round so far (each round shows item count + sample item name + round total). Two huge buttons: 🍽 Add more (drops them back into the menu in "add to existing tab" mode) and 🔒 Close & pay (captures the held pre-auth via /api/stripe-capture, marks every round on this tab collected, writes ONE closed_check covering the lot, clears the stash). "Not me — start fresh" tertiary button for the rare case the previous customer left their browser open.',
      'ADD-MORE-TO-TAB FLOW — when the customer taps 🍽 Add more, the menu loads with `existingTab` context. Building a new cart and tapping checkout DOES NOT trigger Stripe again — instead a simplified "Add to your tab" sheet shows the round total + the running tab total after this round, and a single 🍽 Send to kitchen button. On confirm, a NEW order_queue row is inserted with the SAME payment_intent_id + customer.tab_open=true (so the close-and-capture sweep sums all rounds), source=qr, status=prep so the kitchen prints a fresh ticket for this round.',
      'TABLE SUB-NUMBERING (4.1 / 4.2) — at first tab open, we count existing OPEN tabs (status != collected, customer.tab_open=true) at this table_id. The new tab gets `customer.tableLabel = "{table}.{count+1}"` so two parties at table 4 show up on the operator\'s POS as Table 4.1 and Table 4.2 — separate tabs, separate cards held, separate close-and-charge. Once a tab is captured the slot frees up so the next scan goes back to .1.',
      'CLOSE-AND-PAY CAPTURE — TabResumeScreen 🔒 Close & pay calls /api/stripe-capture with the held payment_intent_id and the actual running total (capped at the pre-auth amount — Stripe rejects over-capture). Every order_queue row sharing that payment_intent_id flips to status=collected, ONE closed_check is written aggregating all rounds (subtotal=running_total, source=qr, table_label=4.x), localStorage stash is cleared, customer is bumped to the OrderTracker showing "✓ Paid £X.XX".',
      'NEXT — commit 3e: phone-fallback lookup for when localStorage is missing (different device, cleared cookies). Customer types their full phone → we look up open tabs at this table by matching phone → soft-verification screen ("your tab from 18:30 with X items, total £Y — that you?") before resuming. Plus the tracker URL doubles as a share/bookmark recovery (already built for paid orders, just needs extending to tabs).',
    ],
  },
  {
  ...
  },
  {
    ...
    changes: [
      ...
      'OVER-AUTH SHORTFALL WARNING — Stripe rejects capture amounts greater than the original pre-auth. If a tab\'s final bill (incl. surcharge) exceeds the £100 hold, the operator gets a clear warning showing exact shortfall before they confirm. Capture is then capped at the pre-auth so they at least get the held amount; the shortfall is left to be collected manually (POS cash, second card etc). Future commit can add cancel-and-recharge for the full amount automatically.',
      'CLOSED_CHECK SURFACES SURCHARGE — the closed_checks row written at force-close time stamps the surcharge into the `service` column and customer.surcharge_applied + customer.tab_closed_at, so reports / receipt re-print make it visible. Source stays "qr" so the BO Reports source filter still slices correctly.',
      '⚠ COMPLETE SQL MIGRATION (idempotent — covers v5.5.145 through v5.5.151): `alter table public.locations add column if not exists qr_payment_mode text default \'pay_now\'; alter table public.locations add column if not exists qr_table_mode text default \'confirm\'; alter table public.locations add column if not exists qr_service_charge_pct numeric(5,2) default 0; alter table public.locations add column if not exists qr_tab_pre_auth_amount numeric(10,2) default 100; alter table public.locations add column if not exists qr_tab_warning_message text; alter table public.locations add column if not exists qr_tab_left_open_surcharge_pct numeric(5,2) default 0; alter table public.locations add column if not exists qr_tab_left_open_surcharge_fixed numeric(10,2) default 0; alter table public.locations add column if not exists qr_tab_force_close_after_minutes integer default 0;`',
      'NB — open-tab requires Vercel env STRIPE_SECRET_KEY (already set for in-store Stripe Test). The /api/stripe-capture endpoint uses the platform secret key with Stripe-Account header to act on the connected account, mirroring the existing edge fn pattern. Email receipts on tab close were deferred to post-launch (see project_post_launch_tasks.md).',
    ],
  },
  {
  ...
  },
  {
    ...
    changes: [
      ...
      'STATUS FLIP MOVED AFTER PRINTS — v5.5.127 flipped order_queue.status \'received\' → \'prep\' right after the kds_tickets insert, BEFORE routePrintJob fired. So the customer\'s tracker said "Preparing" before the kitchen had paper. Moved the flip to after the per-centre print loop, AND gated it on `Object.keys(byCentre).length > 0` — if no centres matched (e.g. routing config not set up yet for this venue) the order genuinely IS still queued so we leave status=\'received\' and let the operator advance manually.',
      'ROUTING DIAGNOSTIC LOG — `[routeKioskOrderPrints] OL-XXX centres matched: N items routed: M` now logs to the console on every run so silent failures (no centres / cats not matched / printer not mapped) surface in DevTools. If you place an online order and prints don\'t fire, check the master device\'s console for this line — `centres matched: 0` means the item categories don\'t map to any centre in your print routing config.',
      'TRACKER POLLS EVERY 5s — 20s was too slow if the customer\'s realtime websocket dropped (anon RLS, network blip, mobile browser tab backgrounded). Polling is the workhorse now — every 5s the OrderTracker re-fetches the order_queue row and only re-renders if status / total / items actually changed (cheap state-equality check). Realtime is still subscribed as a snappy nice-to-have and now logs SUBSCRIBED / CHANNEL_ERROR / TIMED_OUT so we can see whether it\'s working.',
    ],
  },
  {
  ...
  },
  {
    ...
    changes: [
      ...
      'TWO-STEP CHECKOUT — Details/When (step 1) → Pay (step 2). Step 1 collects name/phone/email/(address)/notes + ASAP-or-scheduled time. Continue creates the PaymentIntent on the location\'s connected account, lazy-loads Stripe.js with that account ID, and renders <CardElement> inline. Pay confirms via stripe.confirmCardPayment. ← Edit chip in the header lets the customer go back. Step indicator at the top shows progress.',
      'ORDER WRITTEN ONLY AFTER PAYMENT CLEARS — fixed the v5.5.120 bug where the awaiting_payment placeholder row was visible on the operator queue. The order_queue INSERT (status=\'received\') now happens ONLY after stripe.confirmCardPayment returns paymentIntent.status==\'succeeded\'. If anything errors before that point (validation, intent creation, card decline, network), the kitchen sees nothing — exactly what the user asked for.',
      'POS CHIME + TOAST FIRE FOR ONLINE — realtime.js was watching for an awaiting_payment → received UPDATE that no longer exists in the inline-payment flow. Switched to firing on INSERT for source=\'online\' (alongside the existing source=\'kiosk\' path), so the kitchen hears the chime + sees the toast the moment payment clears. Master device routes prints to production centres for online orders identical to kiosk.',
      'CUSTOMER PROFILE — every online order now flows into the same CRM the in-store flow uses. New attributeOnlineOrder() helper in customerLookup.js mirrors store.attributeOrderToCustomer (which only works in operator context with the Zustand store hydrated): upserts customers row by (org_id, phone), bumps or inserts customer_locations.visit_count + lifetime_revenue, inserts a customer_orders denormalised row with channel=\'online\'. Fire-and-forget — a CRM blip never blocks the confirmation flow. Now every online order / visit / spend appears under the customer\'s profile in BO Customers exactly like in-store orders do.',
      'AUTH FOR ONLINE — customers aren\'t logged in to Supabase, so the edge fn call passes VITE_SUPABASE_ANON_KEY as the bearer (same anon-role JWT the rest of the customer surface uses to read menus / write order_queue under the existing RLS policies). channel=\'online\' on the PaymentIntent so online_markup_percent applies to the platform-fee math.',
      'CLEANUP — removed api/stripe-checkout.js + api/stripe-verify.js, the v5.5.120 OnlineSurface return-from-Stripe useEffect, and the v5.5.120 awaiting_payment filtering in QueueSync.loadQueues + applyQueueRealtimeEvent. Cancellation notice + verify_failed banner state retained for future use but not currently triggered.',
    ],
  },
  {
  ...
  },
  {
    ...
    changes: [
      ...
      'CART GROUPED BY COURSE — MCartSheet now mirrors the desktop POS check view. Each course gets its own section with a coloured header pill ("Course 1 · Starters", "Course 2 · Mains", "Course 3 · Desserts", or "Immediate" for course 0), an item count, and a per-course subtotal. Within a course, pending lines render before sent lines under a small "TO SEND" sub-header so the cashier can spot what is still to fire at a glance. Course headers colour-coded blue/green/orange/purple — same palette the kitchen uses on tickets.',
      'VOID FIXED — was a no-op because the desktop store action expects a manager-PIN object and crashed on null.manager.name. Now: pending items go through removeItem (clean removal, restores daily count), sent table items pass the current staff as the approving party so the void completes and shows up in voidLog. Walk-in fallback also routed through removeItem since the desktop POS doesn\'t void walk-in items via the same path. Manager-PIN gate replaces the staff-fallback in 1D.',
      'ALLERGY FILTER — chip in the MMenu header (top-right). Tap to open MAllergenPicker bottom sheet listing all 14 EU mandatory allergens with circular icon badges (G / C / E / F / P / S / M / N etc). Toggle any to flag. Same store.allergens state the desktop POS uses, so filters set on a phone propagate to all surfaces at the location via realtime. Flagged items in the menu show with red border, red background tint, line-through name, and a "⚠ Contains: gluten, milk" warning line. Server can still tap them (manager override / customer clarification scenarios) — they\'re flagged, not blocked. Active warning banner above the cart bar with chip count + Edit shortcut.',
      'CUSTOMER-FACING TIP PASS — MTender reframed for the moment the phone is handed to the customer. No more method picker, no more "Pay at counter" button. Customer sees: subtotal, four big tip preset cards (10/12.5/15/20%) in a 2×2 grid, "No tip thanks" option, large total readout, big "Confirm" button. On Confirm → auto-runs MCardFlow (Tap to Pay or assigned reader REST). Server-back is a small ghost button at the bottom for accidental swipes — main UI is built around the customer\'s thumbs.',
      'PAY-AT-COUNTER REMOVED — phone is always a card device. Cash belongs at the till where the drawer lives. Server staff route cash payments at the till instead of through the phone. MPayAtCounter.jsx deleted, related routing in MPOSSurface stripped.',
      'NEW FILES this round: MSentConfirm.jsx, MAllergenPicker.jsx. Substantially rewritten: MTender.jsx, MCartSheet.jsx (added CourseGroupedItems), MMenu.jsx (allergen integration), MItemActions.jsx (void-fallback fix). Deleted: MPayAtCounter.jsx.',
    ],
  },
  {
  ...
  },
  {
    ...
    changes: [
      ...
      'NEW FILES (~1300 lines added on top of 1A): MNewOrder.jsx (bottom-sheet order-type picker, profile-aware enabledOrderTypes filtering), MCoversPicker.jsx (full-screen covers wheel with quick chips 1-8), MTableView.jsx (active order screen for a seated table with sent/pending bucketing, total summary card, bottom action bar Add items · Send · Pay), MMenu.jsx (search-first menu browser with category chips, OPTIONS pill on items with modifiers, sticky cart bar at bottom), MItemDetail.jsx (full-screen modifier flow with min/max validation, allergen warning, special-instruction notes 200-char, qty stepper), MCartSheet.jsx (review screen with edit qty + remove on pending items, sent items locked, primary CTA adapts: Send to kitchen · Send & take payment · Take payment).',
      'FLOW STATE MACHINE in MPOSSurface: bottom tabs hide during active flow so the server sees only the task at hand. Screens: newOrder · covers · tableView · menu · item · cart. Back-button behaviour follows the wizard hierarchy. Closing a flow returns to the originating tab.',
      'TABLE SERVICE WIRED END-TO-END: tap a table from the Tables tab → empty? covers picker → seat (calls store.seatTable) → table view. Already-seated table → table view directly. Tap "+ Add items" → menu (with cart bar showing pending items). Tap an item → item detail with modifiers. Add to order → back to menu. Tap cart bar → review → Send to kitchen (calls store.sendToKitchen which fires KDS tickets and routes print jobs to mapped centres via the existing PrintOrchestrator). After sending, returns to table view with items now in "Already sent" section.',
      'WALK-IN / TAKEAWAY / COLLECTION / DELIVERY: + button → pick type → menu (clears walkInOrder, sets orderType) → add items → cart → "Send & take payment" → 1C tender placeholder. Bar tab routes through the same path as takeaway until 1C adds proper bar UI.',
      'REUSES 100% OF THE EXISTING ORDER ENGINE: store.addItem (handles modifiers + per-channel pricing automatically), store.removeItem + store.updateItemQty (clamp at 1, restore daily counts), store.sendToKitchen (creates kds_tickets per centre, routes print_jobs, customer attribution), store.seatTable (opens session, sets activeTableId, surface). The store routes between walkInOrder and tables[].session based on activeTableId — MPOS just sets/clears that and the engine does the right thing.',
      'PROFILE-AWARE: MNewOrder filters order types by deviceConfig.enabledOrderTypes. Bar option hidden if hiddenFeatures includes barTabs. Manager-PIN gating + swipe-actions on cart lines (discount/comp/void/transfer) land in 1D.',
      'WHAT IS NOT IN 1B (intentional, 1C/1D/1E): MTender + MCardFlow (REST + simulated, real Tap to Pay in 1E), MPayAtCounter routing, MReceiptPrompt + email infrastructure, MOrderActions ⋯ menu (apply order discount, split bill, transfer table, fire course, print docket), MManagerPin sheet, swipe-left-on-cart-line action sheet, refunds, voice ordering. All wired into the spec at docs/MPOS-SPEC.md.',
    ],
  },
  {
